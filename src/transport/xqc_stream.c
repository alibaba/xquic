    ...
    if (stream_id == XQC_UNDEFINE_STREAM_ID) { /* sending part */
        ...
        if (stream_type == XQC_CLI_BID || stream_type == XQC_SVR_BID) {
            if (stream_id >= (4 * conn->conn_flow_ctl.fc_max_streams_bidi_can_recv + stream_type)) {
                xqc_log(conn->log, XQC_LOG_ERROR, "|exceed max_streams_bidi_can_recv|stream_id:%ui|can_recv:%ui|",
                        stream_id >> 2, conn->conn_flow_ctl.fc_max_streams_bidi_can_recv);
                XQC_CONN_ERR(conn, TRA_STREAM_LIMIT_ERROR);
                return -XQC_EPROTO;
            }
            /* increase max streams */
            if (((stream_id >> 2) + conn->local_settings.max_streams_bidi/2) >= conn->conn_flow_ctl.fc_max_streams_bidi_can_recv
            ...
            if (stream_id >= (4 * conn->conn_flow_ctl.fc_max_streams_uni_can_recv + stream_type)) {
                xqc_log(conn->log, XQC_LOG_ERROR, "|exceed max_streams_uni_can_recv|stream_id:%ui|can_recv:%ui|",
                        stream_id >> 2, conn->conn_flow_ctl.fc_max_streams_uni_can_recv);
                XQC_CONN_ERR(conn, TRA_STREAM_LIMIT_ERROR);
                return -XQC_EPROTO;
            }
            /* increase max streams */
            if (((stream_id >> 2) + conn->local_settings.max_streams_uni/2) >= conn->conn_flow_ctl.fc_max_streams_uni_can_recv
...
#define __calc_delay(a, b) ((a && b && (a > b))? (a) - (b) : 0)
    ...
    while (stream->reset_at.acked_ranges != NULL) {
        xqc_stream_ack_range_t *range = stream->reset_at.acked_ranges;
        stream->reset_at.acked_ranges = range->next;
        xqc_free(range);
    }
    ...
    /* handle passive bidirectional stream closure */
    if ((stream->stream_type == XQC_CLI_BID && stream->stream_conn->conn_type == XQC_CONN_TYPE_SERVER)
        || (stream->stream_type == XQC_SVR_BID && stream->stream_conn->conn_type == XQC_CONN_TYPE_CLIENT))
    {
        /* decrease passive concurrent bidirectional stream counter */
        if (stream->stream_conn->passive_bidi_stream_cnt > 0) {
            stream->stream_conn->passive_bidi_stream_cnt--;
        }

        /* update flow control window for bidirectional streams */
        ...
        if ((stream->stream_conn->max_stream_id_bidi_remote + 1) >=
                stream->stream_conn->conn_flow_ctl.fc_max_streams_bidi_can_recv)
        {
            stream->stream_conn->conn_flow_ctl.fc_max_streams_bidi_can_recv =
                stream->stream_conn->conn_flow_ctl.fc_max_streams_bidi_recv_wind;
            xqc_write_max_streams_to_packet(stream->stream_conn,
                stream->stream_conn->conn_flow_ctl.fc_max_streams_bidi_can_recv, 1);
        }
    }
    /* handle passive unidirectional stream closure */
    else if ((stream->stream_type == XQC_CLI_UNI && stream->stream_conn->conn_type == XQC_CONN_TYPE_SERVER)
        || (stream->stream_type == XQC_SVR_UNI && stream->stream_conn->conn_type == XQC_CONN_TYPE_CLIENT))
    {
        /* update flow control window for unidirectional streams */
        ...
        { 
            stream->stream_conn->conn_flow_ctl.fc_max_streams_uni_can_recv =
                stream->stream_conn->conn_flow_ctl.fc_max_streams_uni_recv_wind;
            xqc_write_max_streams_to_packet(stream->stream_conn,
                stream->stream_conn->conn_flow_ctl.fc_max_streams_uni_can_recv, 0);
        }
    }
...
xqc_passive_create_stream(xqc_connection_t *conn, xqc_stream_id_t stream_id, void *user_data)
{
    ...
    /* update passive concurrent bidirectional stream statistics */
    if (stream_type == XQC_CLI_BID || stream_type == XQC_SVR_BID) {
        conn->passive_bidi_stream_cnt++;
        if (conn->passive_bidi_stream_cnt > conn->passive_bidi_stream_max) {
            conn->passive_bidi_stream_max = conn->passive_bidi_stream_cnt;
        }
    }
...
xqc_stream_recv(xqc_stream_t *stream, unsigned char *recv_buf, size_t recv_buf_size, uint8_t *fin)
{
    ...
    *fin = 0;
    ...
    if (stream->reset_at.received && !stream->reset_at.reported
        && stream->stream_data_in.next_read_offset
            >= stream->reset_at.recv_size)
    {
        stream->reset_at.reported = XQC_TRUE;
        stream->stream_err = stream->reset_at.recv_error;
        ...
        xqc_stream_closing(stream, stream->reset_at.recv_error);
    }
...
xqc_stream_send(xqc_stream_t *stream, unsigned char *send_data, size_t send_data_size, uint8_t fin)
{
    ...
    if (stream->reset_at.pending) {
        send_data_size = xqc_min(send_data_size,
            stream->reset_at.send_size - stream->stream_send_offset);
        fin = 0;
    }
...
do_buff:
    ...
    if (stream->reset_at.pending
        && stream->stream_send_offset >= stream->reset_at.send_size)
    {
        xqc_int_t reset_ret = xqc_stream_do_reset(stream,
            stream->reset_at.send_error);
        if (reset_ret != XQC_OK) {
            return reset_ret;
        }
    }
...
xqc_stream_set_reliable_size(xqc_stream_t *stream, uint64_t reliable_size)
{
    ...
    if (!(stream->stream_conn->conn_flag & XQC_CONN_FLAG_TLS_HSK_COMPLETED)
        ...
        || stream->stream_send_offset != 0 || stream->reset_at.sent)
    {
        return -XQC_ESTATE;
    }
    stream->reset_at.send_size = reliable_size;
    stream->reset_at.enabled = XQC_TRUE;
    return XQC_OK;
}
...
xqc_stream_do_reset(xqc_stream_t *stream, uint64_t error_code)
{
    xqc_connection_t *conn = stream->stream_conn;
    if (stream->reset_at.sent) {
        return error_code == stream->reset_at.send_error
            ? XQC_OK : -XQC_ESTATE;
    }
    ...
    if (stream->reset_at.enabled) {
        ...
        if (stream->stream_send_offset < stream->reset_at.send_size) {
            stream->reset_at.pending = XQC_TRUE;
            stream->reset_at.send_error = error_code;
            return -XQC_EAGAIN;
        }
        xqc_int_t ret = xqc_write_reset_stream_at_to_packet(conn, stream,
            error_code, stream->stream_send_offset, stream->reset_at.send_size);
        if (ret != XQC_OK) {
            return ret;
        }
        stream->reset_at.sent = XQC_TRUE;
        stream->reset_at.pending = XQC_FALSE;
        stream->reset_at.send_error = error_code;
    ...
    uint64_t length, xqc_bool_t reset_acked)
{
    ...
    if (reset_acked) {
        stream->reset_at.acked = XQC_TRUE;
    }
    uint64_t end = xqc_min(offset + length, stream->reset_at.send_size);
    if (offset < end && end > stream->reset_at.acked_offset) {
        xqc_stream_ack_range_t **at = &stream->reset_at.acked_ranges;
        ...
        *at = range;
        while (stream->reset_at.acked_ranges != NULL
            && stream->reset_at.acked_ranges->start
                <= stream->reset_at.acked_offset)
        {
            range = stream->reset_at.acked_ranges;
            stream->reset_at.acked_offset = xqc_max(
                stream->reset_at.acked_offset, range->end);
            stream->reset_at.acked_ranges = range->next;
            xqc_free(range);
        }
    }
    if (stream->reset_at.sent && stream->reset_at.acked
        && stream->reset_at.acked_offset >= stream->reset_at.send_size
