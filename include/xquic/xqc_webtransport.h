 ...
 *
 * @param engine xquic engine
 * @param cid connection id
 * @param path the path for the WebTransport endpoint (e.g. "/echo")
 * @param authority the authority (e.g. "localhost")
 * @param user_data user data for session callbacks
 * @return XQC_OK on success (session_create_notify will be called when server responds 200)
 */
XQC_EXPORT_PUBLIC_API
xqc_int_t xqc_wt_client_open_session(xqc_engine_t *engine, const xqc_cid_t *cid,
 ...
 * @param fin 1 if this is the last data on this stream
 * @return >= 0 for bytes sent, < 0 for error
 */
XQC_EXPORT_PUBLIC_API
