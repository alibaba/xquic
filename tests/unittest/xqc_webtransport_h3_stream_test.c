...
wt_h3_test_init(xqc_wt_h3_test_t *test, xqc_bool_t bidi)
{
    ...
    test->wt_conn = test->h3c->extension_data;
...
{
    return test->h3c->extension_ops->stream_read(test->h3s,
...
{
    const xqc_h3_extension_ops_t *ops = test->h3c->extension_ops;
    ops->stream_close(test->h3s, test->h3s->extension_data);
    CU_ASSERT_PTR_NULL(test->h3s->extension_data);
    ops->stream_close(test->h3s, test->h3s->extension_data);
    CU_ASSERT_PTR_NULL(test->h3s->extension_data);
...
xqc_test_wt_h3_stream_demux(void)
{
    ...
    for (unsigned bidi = 0; bidi < 2; bidi++) {
        ...
        CU_ASSERT(test.h3s->type == XQC_H3_STREAM_TYPE_EXTENSION);
...
xqc_test_wt_h3_stream_passthrough(void)
{
    ...
    CU_ASSERT_PTR_NOT_NULL(test.h3s->extension_data);
    ...
    CU_ASSERT_FATAL(wt_h3_test_init(&test, XQC_FALSE) == XQC_OK);
    /* An unknown wire type may equal H3's internal extension marker. */
...
xqc_test_wt_h3_stream_backpressure(void)
{
    for (unsigned variant = 0; variant < 4; variant++) {
        ...
        const xqc_h3_extension_ops_t *ops = test.h3c->extension_ops;
        CU_ASSERT(ops->stream_prepare_read(test.h3s,
                  test.h3s->extension_data) == -XQC_EAGAIN);
        ...
        CU_ASSERT(ops->stream_prepare_read(test.h3s,
                  test.h3s->extension_data) == XQC_OK);
