 ...
 * setup, or encode/decode round-trip is needed.
 * ------------------------------------------------------------------------- */
...
/*
 * Create a client connection and plant remembered values into
 * conn->remote_settings as the 0-RTT baseline.
 */
static xqc_connection_t *
xqc_0rtt_test_make_conn(void)
{
    xqc_connection_t *conn = test_engine_connect();
    CU_ASSERT_FATAL(conn != NULL);
...
/* populate params with values equal to the remembered baseline */
static void
...
{
    xqc_connection_t *conn = xqc_0rtt_test_make_conn();

    xqc_transport_params_t params;
    xqc_0rtt_test_init_params(&params);

    xqc_int_t ret = xqc_conn_validate_0rtt_transport_params(conn, &params);
...
xqc_test_0rtt_params_all_increased(void)
{
    ...
    /* increase every parameter -- must succeed */
...
xqc_test_0rtt_params_each_reduced(void)
{
     ...
     * branch triggers TRA_0RTT_TRANS_PARAMS_ERROR.
     */
    struct {
        size_t   tp_offset;
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        *field = cases[i].remembered_val - 1;
        ...
        xqc_int_t ret = xqc_conn_validate_0rtt_transport_params(conn, &params);
        CU_ASSERT_EQUAL(ret, TRA_0RTT_TRANS_PARAMS_ERROR);
