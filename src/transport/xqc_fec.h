...
typedef struct xqc_fec_ctl_s {
    ...
    uint32_t                     fec_send_repair_num;
    uint32_t                     fec_send_required_repair_num;
    xqc_int_t                    fec_max_fin_blk_id;

    xqc_fec_mp_mode_e            fec_mp_mode;
    uint64_t                     fec_rep_path_id;

    uint32_t                     fec_send_block_num;
    uint32_t                     fec_send_symbol_num;           /* src symbols number for current fec process */
...
xqc_int_t xqc_fec_ctl_init_send_params(xqc_connection_t *conn);
