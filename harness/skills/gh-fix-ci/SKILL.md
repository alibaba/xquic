...
3. Map the failure to source using `harness/spec/harness-manifest.yml` and read the required docs before changing code.
...
6. Re-run local validation using `scripts/validate.sh` and `harness/skills/validate/SKILL.md` when feasible. If CI-only, explain why local validation is not equivalent.
