...
typedef std::function<void()> wt_stream_close_func_pt;
typedef std::function<int(WTServer*, WTSession*, WTRequest*)> wt_request_handler_pt;
...
class WTStreamMap {
public:
    std::map<xqc_stream_id_t, wt_stream_close_func_pt> mFuncMap;
    std::mutex m_lock;

public:
    WTStreamMap() {}

    void AddStream(xqc_stream_id_t stream_id, wt_stream_close_func_pt func);
...
public:
    ...
    wt_stream_close_func_pt onClose = nullptr;
...
class WTStream {
public:
    WTStream(wt_quic_stream_type* quicStr, const std::vector<uint64_t>& hdr, wt_stream_close_func_pt closeFunc);
    ~WTStream();
...
public:
    ...
    int        mCloseError = 0;
    std::map<int, wt_stream_close_func_pt> mStreamCtxs;
...
public:
    ...
    int  onUniStreamRecvData(xqc_h3_conn_t* h3_conn, xqc_h3_stream_t* stream, uint8_t* data, size_t size, int* ret) {
        if (onUnistreamDataRecvCallback)
        {
            return onUnistreamDataRecvCallback(h3_conn, stream, data, size, ret);
        }
        return 0;
    }
...
public:
    struct SessionObj {
        int counter; //这个地方的int有待考究
        WTSession* conn;
    };

    struct PendingUnistream {
        xqc_discrete_int_pctx_t     pctx;
        wt_quic_stream_type* stream;
        uint64_t connTracingID;
    };

    wt_stream_close_func_pt mCtxCancel;
    uint64_t                mTimeout;
    std::mutex m_lock;
    std::map<uint64_t, std::map<uint64_t, SessionObj>> mConns;
    std::atomic_int                     mRefCount = 0;
    std::map<wt_quic_stream_type*, PendingUnistream*> mPendingAddUnistream;
...
public:
    ...
    std::tuple<SessionObj*, bool> getOrCreateSession(uint64_t connTracingID, uint64_t sessionId);
    //void handleStream(xqc_stream_t* str, SessionObj* sess);
    //void handleUniStream(xqc_stream_t* str, SessionObj* sess);
    WTSession* addSession(WTConnection *qconn, uint64_t sessionId, wt_quic_stream_type*requestStr);
    void close();

    int  onUniStreamRecvData(xqc_h3_conn_t* h3_conn, xqc_h3_stream_t* stream, uint8_t* data, size_t size, int* ret);
    // int  DatagramRecvData(xqc_h3_conn_t* h3_conn, xqc_h3_stream_t* stream, uint8_t* data, size_t size, int* ret);
};
...
public:
    ...
    int openLogfiles();
    int closeLogfiles();
    void writeLogFile(xqc_log_level_t lvl, const void* buf, size_t size);
...
public:
    ...
    wt_stream_close_func_pt  mCtxCancel;
    int           mInitErr = 0;
    WTSessionManager* mSessionManager = nullptr;
...
void startWebtransportServer(int argc, char* argv[], WTServer *server);
void stopWebtransportServer();
void Send_datagram(WTConnection *user_conn , WTServer* ctx);