...
void on_session_setup(xqc_moq_user_session_t *user_session, char *extdata)
{
    ...
    xqc_moq_track_t *track3 = xqc_moq_track_create(session, "example/audio", "mic", XQC_MOQ_TRACK_DEFAULT,
                                                    NULL, XQC_MOQ_CONTAINER_LOC, XQC_MOQ_TRACK_FOR_PUB);
    if (track3 == NULL) {
                            ...
                            xqc_moq_subscribe_namespace_msg_t *subscribe_namespace)
{
    ...
    /* Fast RTT test: send first subgroup object immediately with the default track */
    if (user_conn->default_track) {
        const char *fast_msg = "test fast rtt";
        xqc_moq_stream_t *used_stream = xqc_moq_default_track_send_on_stream(
            user_conn->default_track,
            NULL,                  /* create new stream if needed */
            0,                     /* subgroup_id */
            (uint8_t *)fast_msg,
            strlen(fast_msg) + 1,  /* include trailing nul for readability */
            1                      /* is_first: allow creating a new stream */
        );
        if (used_stream) {
            char ts2[32];
            xqc_get_timestamp(ts2, sizeof(ts2));
            printf("%s [SERVER] Sent immediate fast-RTT subgroup on default track: '%s'\n", ts2, fast_msg);
        }
    }
}
