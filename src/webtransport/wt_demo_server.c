...
#define SOURCE_DIR "."
#define PRIV_KEY_PATH   "certs/localhost.key"
#define CERT_PEM_PATH   "certs/localhost.crt"
...
// server_accept callback: required for xquic to accept new QUIC connections
static int
wt_svr_accept(xqc_engine_t *engine, xqc_connection_t *conn,
...
wt_svr_init_conn_settings(WT_Server *server)
{
    ...
    switch ( server->wt_NetConfig->cc ) {
        case CC_TYPE_BBR: ccc = xqc_bbr_cb; break;
        case CC_TYPE_CUBIC: ccc = xqc_cubic_cb; break;
...
wt_svr_socket_read_handler(WT_Server *server, int fd)
{
    ...
    do {
        ...
        uint64_t  recv_time = xqc_monotonic_timestamp();
