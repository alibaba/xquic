...
void
xqc_packet_mask_init(xqc_connection_t *conn, xqc_int_t k, xqc_int_t n)
{
    // init packet mask with default block size and code rate
    uint8_t count, *output = NULL, table[XQC_MAX_PM_SIZE];
    const uint8_t *mask_entry = NULL, *tbl_p = table;
    uint16_t tbl_len, repair_num, increment;
    ...
    for (uint32_t i = 0; i < repair_num; i++) {
        output = conn->fec_ctl->decode_matrix[i];
...
{
    uint32_t src_syb_num, total_syb_num, repair_num, symbol_idx;
    ...
    total_syb_num = conn->conn_settings.fec_params.fec_max_symbol_num_per_block;
    repair_num = conn->fec_ctl->fec_send_required_repair_num;
    ...
    for (uint32_t i = 0; i < repair_num; i++) {
        // 比对pm[i]与当前src symbol 的symbol idx, 若与值为1，则进行xor计算
        pm_p = conn->fec_ctl->decode_matrix[i];
        rpr_key_p = conn->fec_ctl->fec_send_repair_key[i].payload;
        if (*(rpr_key_p + pm_offset) & symbol_flag) {
            ...
            if (ret == XQC_OK) {
                ...
                *(rpr_key_p + pm_offset) |= symbol_flag;
                xqc_set_object_value(&conn->fec_ctl->fec_send_repair_key[i], 1, rpr_key_p, pm_size);
            }
        }
    }
    return ret;
...
xqc_packet_mask_encode(xqc_connection_t *conn, unsigned char *stream, size_t st_size, unsigned char **outputs)
{
    ...
    ret = xqc_pm_code_symbols(conn, stream, st_size, outputs);
    if (ret != XQC_OK) {
