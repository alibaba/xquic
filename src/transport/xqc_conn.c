...
xqc_conn_check_transport_params(xqc_connection_t *conn, const xqc_transport_params_t *params)
{
    ...
    if (conn->conn_type == XQC_CONN_TYPE_SERVER) {
         ...
         *
         * For server, conn->original_dcid stores the SCID of the client's
         * first Initial packet (see xqc_conn_server_create), which is exactly
         * the value the client must echo via initial_source_connection_id.
         */
