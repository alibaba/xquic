...
xqc_client_bytestream_send(xqc_h3_ext_bytestream_t *h3_bs, user_stream_t *user_stream)
{
    ...
    if (g_test_case == 303) {
        //send pure fin on bytestream
        xqc_h3_ext_bytestream_send(h3_bs, NULL, 0, 1, XQC_DATA_QOS_HIGH);
        return 0;
    }
    ...
    /* send body */
    if (user_stream->send_offset < user_stream->send_body_len) {
        ret = xqc_h3_ext_bytestream_send(h3_bs, user_stream->send_body + user_stream->send_offset, user_stream->send_body_len - user_stream->send_offset, fin, XQC_DATA_QOS_HIGH);
...
int main(int argc, char *argv[]) {
    ...
    int ch = 0;
    while ((ch = getopt_long(argc, argv, "a:p:P:n:c:Ct:T:1s:w:r:l:Ed:u:H:h:Gx:6NMR:i:V:q:o:fe:F:D:b:B:J:Q:U:Ay", long_opts, NULL)) != -1) {
        switch (ch) {
        case 'U':
