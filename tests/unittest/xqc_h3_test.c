 ...
 * deterministically.
 *
 * The fix has four guards (in order):
 *   G1: h3s->type == XQC_H3_STREAM_TYPE_CONTROL
 *   G2: !(conn->conn_flag & XQC_CONN_FLAG_CLOSING_NOTIFY)   (avoid teardown noise)
 *   G3: conn->conn_state < XQC_CONN_STATE_CLOSING            (state-machine backup)
 *   G4: !(h3s->flags & XQC_HTTP3_STREAM_FLAG_ACTIVELY_CLOSED) (passive close only)
 *
 * Cases below cover one positive path (all guards pass -> error raised) plus
 * four negative paths, each isolating one guard.
 */
...
xqc_test_h3_critical_stream_close_one(xqc_h3_stream_type_t stream_type,
    int set_actively_closed, int set_closing_notify, int set_conn_state_closing,
...
xqc_test_h3_critical_stream_close()
{
    ...
    xqc_test_h3_critical_stream_close_one(XQC_H3_STREAM_TYPE_CONTROL,
            ...
            /* conn_state_closing */ 0,
            /* expected_err   */ H3_CLOSED_CRITICAL_STREAM);

    /*
     * Case 2 (negative, guard G4): local stack actively closed the control
    ...
    /*
     * Case 3 (negative, guard G2): CLOSING_NOTIFY flag is set, indicating
     * connection teardown is in progress. Stream cleanup here is part of
     * normal teardown — no error must surface.
     */
    xqc_test_h3_critical_stream_close_one(XQC_H3_STREAM_TYPE_CONTROL,
    ...
    /*
     * Case 5 (negative, guard G3): conn_state has reached CLOSING but
     * CLOSING_NOTIFY flag is NOT yet set. This is the real-world scenario
     * discovered during case_test validation — some connections enter the
     * CLOSING state machine phase before the flag is propagated. The
     * conn_state check acts as a backup guard to prevent false positives.
     */
    xqc_test_h3_critical_stream_close_one(XQC_H3_STREAM_TYPE_CONTROL,
            /* actively_closed */ 0, /* closing_notify */ 0,
            /* conn_state_closing */ 1,
            /* expected_err   */ 0);
}
