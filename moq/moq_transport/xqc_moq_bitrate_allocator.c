...
void xqc_moq_init_bitrate(xqc_moq_session_t *session)
{
...
xqc_moq_target_bitrate(xqc_moq_session_t *session)
{
    ...
    /* use quic_conn, not trans_conn — trans_conn type depends on transport */
    conn = session->quic_conn;
...
void 
xqc_moq_bitrate_alloc_on_frame_acked(xqc_moq_session_t *session, xqc_moq_track_t *track,
    ...
    uint64_t stream_len, uint64_t seq_num)
{
    ...
    if (bitrate_alloc->latest_delay > DELAY_THRESHOLD_MAX || (bitrate_alloc->latest_delay > DELAY_THRESHOLD_MEDIUM && is_model_done && model.slope > threshold && model.slope > bitrate_alloc->prev_slope))
    {
        bitrate_alloc->delay_under_cnt = 0;
        bitrate_alloc->delay_over_cnt++;
        bitrate_alloc->prev_slope = model.slope;

        if (bitrate_alloc->delay_over_cnt >= DELAY_OVER_THRESHOLD && now > bitrate_alloc->last_bitrate_decrease_time + BITRATE_DECREASE_INTERVAL)
        {
    ...
    else if (bitrate_alloc->latest_delay <= DELAY_THRESHOLD_MIN || (bitrate_alloc->latest_delay <= DELAY_THRESHOLD_MEDIUM && is_model_done && model.slope < threshold && model.slope > -threshold))
    {
        bitrate_alloc->delay_over_cnt = 0;
        bitrate_alloc->delay_under_cnt++;
        bitrate_alloc->prev_slope = 0.0;

        if (bitrate_alloc->delay_under_cnt >= DELAY_UNDER_THRESHOLD && now > bitrate_alloc->last_bitrate_change_time + BITRATE_INCREASE_INTERVAL)
        {
...
finish:
    ...
    if (bitrate_alloc->target_bitrate != bitrate_alloc->prev_target_bitrate)
    {
        ...
        {
            session->session_callbacks.on_bitrate_change(session->user_session, track, track_info,
                bitrate_alloc->target_bitrate);
