...
void
xqc_submatrix(uint32_t row_min, uint32_t row_max,
    uint32_t col_min, uint32_t col_max,
    uint32_t col_max_sub, uint32_t col_max_matrix,
    unsigned char *submatrix, unsigned char *matrix)
{
    xqc_memset(submatrix, 0, col_max_sub * row_max);
    for (uint32_t row_i = row_min; row_i < row_max; row_i++) {
        for (uint32_t col_i = col_min; col_i < col_max; col_i++) {
    ...
    unsigned char (*output)[2*XQC_MAX_MT_ROW])
{
    ...
    for (uint32_t row_i = 0; row_i < left_rows; row_i++) {
        for (uint32_t col_i = 0; col_i < left_cols; col_i++) {
            output[row_i][col_i] = left[row_i][col_i];
        }
        for (uint32_t col_i = 0; col_i < right_cols; col_i++) {
...
xqc_gaussian_elimination(unsigned char rows, unsigned char cols,
    unsigned char (*output)[2*XQC_MAX_MT_ROW])
{
    uint32_t row_i, col_i, max_row, i, tmp, inv, row_above;
    ...
    unsigned char (*output)[XQC_MAX_MT_ROW])
{
    ...
    for (uint32_t row_i = 0; row_i < left_row; row_i++) {
        for(uint32_t col_i = 0; col_i < right_col; col_i++) {
            value = 0;
