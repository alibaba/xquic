...
typedef struct xqc_io_svr_net_config_s {
...
typedef struct xqc_io_svr_quic_config_s {
    /* cipher config */
...
/* environment config */
typedef struct xqc_io_svr_env_config_s {
    /* log path */
...
} xqc_io_svr_env_config_t;


typedef struct xqc_io_svr_args_s {
    /* network args */
    xqc_io_svr_net_config_t    net_cfg;

    /* quic args */
    xqc_io_svr_quic_config_t   quic_cfg;

    /* environment args */
    xqc_io_svr_env_config_t    env_cfg;
} xqc_io_svr_args_t;



typedef struct xqc_io_svr_ctx_s {
    xqc_engine_t        *engine;
    ...
    xqc_io_svr_args_t *args;
} xqc_io_svr_ctx_t;


typedef struct xqc_io_svr_user_conn_s {
    struct event          *ev_timeout;
    struct sockaddr_in6     peer_addr;
    socklen_t               peer_addrlen;
    xqc_cid_t               cid;
    xqc_io_svr_ctx_t     *ctx;
} xqc_io_svr_user_conn_t;

typedef struct xqc_io_svr_resource_s {
    FILE       *fp;
...
} xqc_io_svr_resource_t;


#define REQ_BUF_SIZE        2048
#define REQ_H3_BODY_SIZE    1024 * 1024
typedef struct xqc_io_svr_user_stream_s {
    ...
    // xqc_io_svr_user_conn_t         *conn;
    xqc_io_svr_resource_t     res;  /* resource info */
} xqc_io_svr_user_stream_t;


/* the global unique server context */
xqc_io_svr_ctx_t svr_ctx;
...
void
xqc_io_svr_set_event_timer(xqc_msec_t wake_after, void *eng_user_data)
{
    xqc_io_svr_ctx_t *ctx = (xqc_io_svr_ctx_t *)eng_user_data;
...
int
xqc_io_svr_accept(xqc_engine_t *engine, xqc_connection_t *conn, const xqc_cid_t *cid,
    void *eng_user_data)
{
    DEBUG;
 ...
 */
int
xqc_io_svr_open_log_file(xqc_io_svr_ctx_t *ctx)
{
...
int
xqc_io_svr_close_log_file(xqc_io_svr_ctx_t *ctx)
{
    if (ctx->log_fd <= 0) {
        return -1;
    }
...
void
xqc_io_svr_write_log_file(xqc_log_level_t lvl, const void *buf, size_t size, void *eng_user_data)
{
    xqc_io_svr_ctx_t *ctx = (xqc_io_svr_ctx_t*)eng_user_data;
    if (ctx->log_fd <= 0) {
        return;
 ...
 */
int
xqc_io_svr_open_keylog_file(xqc_io_svr_ctx_t *ctx)
{
...
int
xqc_io_svr_close_keylog_file(xqc_io_svr_ctx_t *ctx)
{
    if (ctx->keylog_fd <= 0) {
        return -1;
    }
...
void
xqc_io_svr_keylog_cb(const char *line, void *eng_user_data)
{
    xqc_io_svr_ctx_t *ctx = (xqc_io_svr_ctx_t*)eng_user_data;
    if (ctx->keylog_fd <= 0) {
 ...
 ******************************************************************************/
void
xqc_io_svr_tls_key_cb(char *key, void *conn_user_data)
{
    xqc_io_svr_user_conn_t *user_conn = (xqc_io_svr_user_conn_t*)conn_user_data;
...
void
xqc_io_svr_conn_update_cid_notify(xqc_connection_t *conn, const xqc_cid_t *retire_cid,
    const xqc_cid_t *new_cid, void *user_data)
{
    xqc_io_svr_user_conn_t *user_conn = (xqc_io_svr_user_conn_t *)user_data;
...
void
xqc_io_svr_close_user_stream_resource(xqc_io_svr_user_stream_t * user_stream)
{
...
int
xqc_io_svr_hq_conn_create_notify(xqc_hq_conn_t *hqc, const xqc_cid_t *cid, void *conn_user_data)
{
    DEBUG;
    xqc_io_svr_user_conn_t *user_conn = calloc(1, sizeof(xqc_io_svr_user_conn_t));
...
int
xqc_io_svr_hq_conn_close_notify(xqc_hq_conn_t *conn, const xqc_cid_t *cid, void *conn_user_data)
{
    DEBUG;
...
void
xqc_io_svr_hq_conn_handshake_finished(xqc_hq_conn_t *conn, void *conn_user_data)
{
    DEBUG;
    // printf("xqc_io_svr_conn_handshake_finished, user_data: %p, conn: %p\n", conn_user_data, conn);
    xqc_io_svr_user_conn_t *user_conn = (xqc_io_svr_user_conn_t *)conn_user_data;
}


int
xqc_io_svr_send_rsp_resource(xqc_io_svr_user_stream_t *user_stream, char *data, ssize_t len,
    int fin)
{
...
int
xqc_io_svr_hq_req_create_notify(xqc_hq_request_t *hqr, void *req_user_data)
{
    DEBUG;
    xqc_io_svr_user_stream_t *user_stream = calloc(1, sizeof(xqc_io_svr_user_stream_t));
...
int
xqc_io_svr_hq_req_close_notify(xqc_hq_request_t *hqr, void *req_user_data)
{
    DEBUG;
    xqc_io_svr_user_stream_t *user_stream = (xqc_io_svr_user_stream_t*)req_user_data;
    free(user_stream);
    return 0;
}
 ...
 */
int
xqc_io_svr_hq_send_file(xqc_hq_request_t *hqr, xqc_io_svr_user_stream_t *user_stream)
{
    int ret = 0;
    xqc_io_svr_resource_t *res = &user_stream->res;
    while (res->total_offset < res->total_len) {   /* still have bytes to be sent */
        ...
        ret = xqc_io_svr_send_rsp_resource(user_stream, send_buf, send_len, fin);
        if (ret > 0) {
            res->buf_offset += ret;
...
void
xqc_io_svr_handle_hq_request(xqc_io_svr_user_stream_t *user_stream, xqc_hq_request_t *hqr,
    char *resource, ssize_t len)
{
    ...
    /* begin to send file */
    ret = xqc_io_svr_hq_send_file(hqr, user_stream);
    if (ret == 0) {
        return;
    }

handle_error:
    xqc_io_svr_close_user_stream_resource(user_stream);
}


int
xqc_io_svr_hq_req_read_notify(xqc_hq_request_t *hqr, void *req_user_data)
{
    DEBUG;
    unsigned char fin = 0;
    xqc_io_svr_user_stream_t *user_stream = (xqc_io_svr_user_stream_t *)req_user_data;
    ...
    if (fin) {
        xqc_io_svr_handle_hq_request(user_stream, hqr, user_stream->recv_buf,
...
int
xqc_io_svr_hq_req_write_notify(xqc_hq_request_t *hqr, void *req_user_data)
{
    DEBUG;
    //printf("xqc_io_svr_hq_req_write_notify user_data: %p\n", user_data);
    xqc_io_svr_user_stream_t *user_stream = (xqc_io_svr_user_stream_t*)req_user_data;
    int ret = xqc_io_svr_hq_send_file(hqr, user_stream);
    if (ret != 0) {
...
int
xqc_io_svr_h3_conn_create_notify(xqc_h3_conn_t *h3_conn, const xqc_cid_t *cid,
    void *conn_user_data)
{
    DEBUG;
    xqc_io_svr_ctx_t *ctx = (xqc_io_svr_ctx_t*)conn_user_data;
...
/*
    printf("xqc_io_svr_h3_conn_create_notify, user_conn: %p, h3_conn: %p, ctx: %p\n", user_conn,
        h3_conn, ctx);
*/
...
int
xqc_io_svr_h3_conn_close_notify(xqc_h3_conn_t *h3_conn, const xqc_cid_t *cid,
    void *conn_user_data)
{
    DEBUG;
    xqc_io_svr_user_conn_t *user_conn = (xqc_io_svr_user_conn_t*)conn_user_data;
...
void 
xqc_io_svr_h3_conn_handshake_finished(xqc_h3_conn_t *h3_conn, void *conn_user_data)
{
    DEBUG;
    xqc_io_svr_user_conn_t *user_conn = (xqc_io_svr_user_conn_t *)conn_user_data;
...
int
xqc_io_svr_h3_request_create_notify(xqc_h3_request_t *h3_request, void *strm_user_data)
{
    DEBUG;
/*
    printf("xqc_io_svr_h3_request_create_notify, h3_request: %p, strm_user_data: %p\n",
        h3_request, strm_user_data);
*/
    xqc_io_svr_user_stream_t *user_stream = calloc(1, sizeof(*user_stream));
    user_stream->h3_request = h3_request;

    // user_stream->conn = (xqc_io_svr_user_conn_t*)strm_user_data;
...
int
xqc_io_svr_h3_request_close_notify(xqc_h3_request_t *h3_request, void *strm_user_data)
{
    DEBUG;
    xqc_io_svr_user_stream_t *user_stream = (xqc_io_svr_user_stream_t*)strm_user_data;
    xqc_io_svr_close_user_stream_resource(user_stream);
    free(user_stream);
...
void
xqc_io_svr_set_rsp_header_value_str(xqc_http_headers_t *rsp_hdrs, H3_HDR_TYPE hdr_type, char *v)
{
...
void
xqc_io_svr_set_rsp_header_value_int(xqc_http_headers_t *rsp_hdrs, H3_HDR_TYPE hdr_type, int v)
{
...
int
xqc_io_svr_request_send_body(xqc_io_svr_user_stream_t *user_stream, char *data, ssize_t len,
    int fin)
{
...
int
xqc_io_svr_send_body(xqc_io_svr_user_stream_t *user_stream)
{
    int ret = 0;
    xqc_io_svr_resource_t *res = &user_stream->res;
    while (res->total_offset < res->total_len) {    /* still have bytes to be sent */
        ...
        /* send buf */
        int fin = send_len + res->total_offset == res->total_len ? 1 : 0;
        ret = xqc_io_svr_request_send_body(user_stream, send_buf, send_len, fin);
...
int
xqc_io_svr_handle_h3_request(xqc_io_svr_user_stream_t *user_stream,
    xqc_http_headers_t *req_hdrs)
{
    ...
    if (NULL == user_stream->res.fp) {
        printf("error open file [%s]\n", file_path);
        xqc_io_svr_set_rsp_header_value_int(&rsp_hdrs, H3_HDR_STATUS, 404);
    ...
    if (NULL == user_stream->res.buf) {
        ...
        xqc_io_svr_set_rsp_header_value_int(&rsp_hdrs, H3_HDR_STATUS, 500);
        goto h3_handle_error;
    }
    user_stream->res.buf_size = READ_FILE_BUF_LEN;
    ...
    xqc_io_svr_set_rsp_header_value_int(&rsp_hdrs, H3_HDR_CONTENT_LENGTH,
        user_stream->res.total_len);
    xqc_io_svr_set_rsp_header_value_int(&rsp_hdrs, H3_HDR_STATUS, 200);
    ...
    /* begin to send file */
    ret = xqc_io_svr_send_body(user_stream);
    if (ret == 0) {
        return 0;
    }
...
int
xqc_io_svr_h3_request_read_notify(xqc_h3_request_t *h3_request, xqc_request_notify_flag_t flag,
    void *strm_user_data)
{
    DEBUG;
    int ret;
    ...
    if (fin) {
        xqc_io_svr_handle_h3_request(user_stream, headers);
    }
...
int
xqc_io_svr_h3_request_write_notify(xqc_h3_request_t *h3_request, void *strm_user_data)
{
    DEBUG;
    xqc_io_svr_user_stream_t *user_stream = (xqc_io_svr_user_stream_t *)strm_user_data;
    int ret = xqc_io_svr_send_body(user_stream);
...
ssize_t
xqc_io_svr_write_socket(const unsigned char *buf, size_t size, const struct sockaddr *peer_addr,
    socklen_t peer_addrlen, void *conn_user_data)
{
    ...
    do {
        ...
        if (res < 0) {
            printf("xqc_io_svr_write_socket err %zd %s, fd: %d\n", res, strerror(errno), fd);
            if (errno == EAGAIN) {
...
void
xqc_io_svr_socket_write_handler(xqc_io_svr_ctx_t *ctx, int fd)
{
    DEBUG
}

void
xqc_io_svr_socket_read_handler(xqc_io_svr_ctx_t *ctx, int fd)
{
    DEBUG;
    ssize_t recv_sum = 0;
    struct sockaddr_in6 peer_addr;
...
static void
xqc_io_svr_socket_event_callback(int fd, short what, void *arg)
{
    //DEBUG;
    xqc_io_svr_ctx_t *ctx = (xqc_io_svr_ctx_t *)arg;
    if (what & EV_WRITE) {
        xqc_io_svr_socket_write_handler(ctx, fd);

    } else if (what & EV_READ) {
        xqc_io_svr_socket_read_handler(ctx, fd);
...
static int
xqc_io_svr_init_socket(int family, uint16_t port, 
...
static int
xqc_io_svr_create_socket(xqc_io_svr_ctx_t *ctx, xqc_io_svr_net_config_t* cfg)
{
    /* ipv4 socket */
    ...
    ctx->fd = xqc_io_svr_init_socket(AF_INET, cfg->port, (struct sockaddr*)&ctx->local_addr, 
        ctx->local_addrlen);
    ...
    ctx->fd6 = xqc_io_svr_init_socket(AF_INET6, cfg->port, (struct sockaddr*)&ctx->local_addr6, 
        ctx->local_addrlen6);
...
static void
xqc_io_svr_engine_callback(int fd, short what, void *arg)
{
    xqc_io_svr_ctx_t *ctx = (xqc_io_svr_ctx_t *) arg;
...
void
xqc_io_svr_usage(int argc, char *argv[])
{
    char *prog = argv[0];
...
void
xqc_io_svr_init_0rtt(xqc_io_svr_args_t *args)
{
...
void
xqc_io_svr_init_args(xqc_io_svr_args_t *args)
{
    memset(args, 0, sizeof(xqc_io_svr_args_t));
...
void
xqc_io_svr_parse_args(int argc, char *argv[], xqc_io_svr_args_t *args)
{
    int ch = 0;
...
void
xqc_io_svr_init_callback(xqc_engine_callback_t *cb, xqc_transport_callbacks_t *transport_cbs,
    xqc_io_svr_args_t* args)
{
    static xqc_engine_callback_t callback = {
        .set_event_timer = xqc_io_svr_set_event_timer,
        .log_callbacks = {
            .xqc_log_write_err = xqc_io_svr_write_log_file,
            .xqc_log_write_stat = xqc_io_svr_write_log_file
        },
        .keylog_cb = xqc_io_svr_keylog_cb,
    };


    static xqc_transport_callbacks_t tcb = {
        .server_accept = xqc_io_svr_accept,
        .write_socket = xqc_io_svr_write_socket,
        .conn_update_cid_notify = xqc_io_svr_conn_update_cid_notify,
    };
...
/* init server ctx */
void
xqc_io_svr_init_ctx(xqc_io_svr_ctx_t *ctx, xqc_io_svr_args_t *args)
{
    memset(ctx, 0, sizeof(xqc_io_svr_ctx_t));
    ctx->current_fd = -1;
    ctx->args = args;
    xqc_io_svr_open_log_file(ctx);
    xqc_io_svr_open_keylog_file(ctx);
}

/* init ssl config */
void
xqc_io_svr_init_ssl_config(xqc_engine_ssl_config_t *cfg, xqc_io_svr_args_t *args)
{
...
void
xqc_io_svr_init_conn_settings(xqc_io_svr_args_t *args)
{
...
int
xqc_io_svr_init_alpn_ctx(xqc_io_svr_ctx_t *ctx)
{
    int ret = 0;

    xqc_hq_callbacks_t hq_cbs = {
        .hqc_cbs = {
            .conn_create_notify = xqc_io_svr_hq_conn_create_notify,
            .conn_close_notify = xqc_io_svr_hq_conn_close_notify,
        },
        .hqr_cbs = {
            .req_create_notify = xqc_io_svr_hq_req_create_notify,
            .req_close_notify = xqc_io_svr_hq_req_close_notify,
            .req_read_notify = xqc_io_svr_hq_req_read_notify,
            .req_write_notify = xqc_io_svr_hq_req_write_notify,
        }
    };
    ...
    xqc_h3_callbacks_t h3_cbs = {
        .h3c_cbs = {
            .h3_conn_create_notify = xqc_io_svr_h3_conn_create_notify,
            .h3_conn_close_notify = xqc_io_svr_h3_conn_close_notify,
            .h3_conn_handshake_finished = xqc_io_svr_h3_conn_handshake_finished,
        },
        .h3r_cbs = {
            .h3_request_create_notify = xqc_io_svr_h3_request_create_notify,
            .h3_request_close_notify = xqc_io_svr_h3_request_close_notify,
            .h3_request_read_notify = xqc_io_svr_h3_request_read_notify,
            .h3_request_write_notify = xqc_io_svr_h3_request_write_notify,
        }
    };
...
int
xqc_io_svr_init_xquic_engine(xqc_io_svr_ctx_t *ctx, xqc_io_svr_args_t *args)
{
    /* init engine ssl config */
    xqc_engine_ssl_config_t cfg = {0};
    xqc_io_svr_init_ssl_config(&cfg, args);

    /* init engine callbacks */
    xqc_engine_callback_t callback;
    xqc_transport_callbacks_t transport_cbs;
    xqc_io_svr_init_callback(&callback, &transport_cbs, args);

    /* init server connection settings */
    xqc_io_svr_init_conn_settings(args);
    ...
    if (xqc_io_svr_init_alpn_ctx(ctx) < 0) {
...
void
xqc_io_svr_free_ctx(xqc_io_svr_ctx_t *ctx)
{
    xqc_io_svr_close_keylog_file(ctx);
    xqc_io_svr_close_log_file(ctx);
...
main(int argc, char *argv[])
{
    ...
    xqc_io_svr_args_t *args = calloc(1, sizeof(xqc_io_svr_args_t));
    xqc_io_svr_init_args(args);
    xqc_io_svr_parse_args(argc, argv, args);

    /* init server ctx */
    xqc_io_svr_ctx_t *ctx = &svr_ctx;
    xqc_io_svr_init_ctx(ctx, args);

    /* engine event */
    struct event_base *eb = event_base_new();
    ctx->ev_engine = event_new(eb, -1, 0, xqc_io_svr_engine_callback, ctx);

    if (xqc_io_svr_init_xquic_engine(ctx, args) < 0) {
        return -1;
    }

    /* init socket */
    int ret = xqc_io_svr_create_socket(ctx, &args->net_cfg);
    if (ret < 0) {
    ...
    ctx->ev_socket = event_new(eb, ctx->fd, EV_READ | EV_PERSIST,
        xqc_io_svr_socket_event_callback, ctx);
