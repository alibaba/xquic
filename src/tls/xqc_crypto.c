    ...
    uint8_t *pktno, uint8_t *end)
{
    ...
    /*
     * RFC 9001 Section 5.4.2: header protection samples 16 bytes from a fixed offset
     * after the packet number. Endpoints MUST discard packets that are not long
     * enough to contain a complete sample. Without this check, hp_mask below would
     * read XQC_HP_SAMPLELEN bytes starting at `sample`, which could read past `end`.
     */
    ...
    uint8_t *pktno, uint8_t *end)
{
     ...
     * RFC 9001 Section 5.4.2: header protection samples 16 bytes from a fixed offset
     * after the packet number. Endpoints MUST discard packets that are not long
     * enough to contain a complete sample. This check must happen BEFORE hp_mask,
     * otherwise hp_mask would read XQC_HP_SAMPLELEN bytes past `end`.
     */
    if (sample + XQC_HP_SAMPLELEN > end) {
        xqc_log(crypto->log, XQC_LOG_ERROR,
                "|illegal pkt, hp sample exceed buffer|sample_off:%uz|remain:%uz|",
