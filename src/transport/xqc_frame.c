    ...
    while (packet_in->pos < packet_in->last) {
        ...
        if (conn->conn_state == XQC_CONN_STATE_CLOSING) {
            ...
            if (frame_type != 0x1c && frame_type != 0x1d
                && frame_type != 0x15228c00 && frame_type != 0x15228c01)
            {
...
xqc_process_path_challenge_frame(xqc_connection_t *conn, xqc_packet_in_t *packet_in)
{
    ...
    if (path == NULL) {
        if (conn->conn_type == XQC_CONN_TYPE_SERVER) {
            ...
            path = xqc_conn_create_path_inner(conn, &packet_in->pi_pkt.pkt_dcid, NULL, XQC_APP_PATH_STATUS_AVAILABLE, path_id);
            if (path == NULL) {
                xqc_log(conn->log, XQC_LOG_ERROR, "|xqc_conn_create_path_inner err|%ui|", path_id);
...
xqc_process_max_path_id_frame(xqc_connection_t *conn, xqc_packet_in_t *packet_in)
{
    ...
    xqc_log(conn->log, XQC_LOG_DEBUG,
            "|max_path_id:%ui|prev_max_path_id:%ui|", max_path_id, conn->remote_current_max_paths);
