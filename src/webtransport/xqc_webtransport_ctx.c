...
/* file-scope singleton: set once in xqc_wt_ctx_init, read by conn_create callback */
static xqc_wt_ctx_t *s_wt_ctx = NULL;
    ...
    size_t data_len, uint64_t unix_ts)
{
    ...
    if (wt_ctx->dgram_cbs && wt_ctx->dgram_cbs->dgram_read_notify) {
...
/* WT settings IDs are defined in src/http3/xqc_h3_defs.h:
    ...
    void *user_data, uint64_t recv_time)
{
    ...
    xqc_wt_ctx_t  *wt_ctx  = (wt_conn && wt_conn->wt_ctx) ? wt_conn->wt_ctx : s_wt_ctx;

    /* if application registered a datagram callback, let it handle */
    if (wt_ctx && wt_ctx->dgram_cbs && wt_ctx->dgram_cbs->dgram_read_notify) {
    ...
    /* fallback: echo datagram back (server default) */
    uint64_t dgram_id = 0;
    xqc_h3_ext_datagram_send(conn, (void *)data, data_len, &dgram_id, XQC_DATA_QOS_HIGH);
}
    ...
    uint64_t data_recv_time)
{
    ...
    /* skip WT session_id prefix on first read (already consumed by H3 frame parsing) */
    /* the bytestream layer strips the frame type for us, but we still have the session_id varint */

    /* notify application's bidistream_read callback if registered */
    if (wt_ctx->stream_cbs && wt_ctx->stream_cbs->wt_bidistream_read_notify) {
        xqc_wt_session_t *wt_session = wt_conn->wt_session;
        wt_ctx->stream_cbs->wt_bidistream_read_notify(NULL, wt_session,
            (void *)data, data_len, NULL);
        return 0;
    }

    /* fallback: echo data back via raw QUIC stream (server default behavior) */
    xqc_stream_id_t stream_id = xqc_h3_ext_bytestream_id(h3_ext_bs);
    ...
    if (stream) {
        ssize_t sent = xqc_stream_send(stream, (unsigned char *)data, data_len, fin);
        if (sent < 0) {
            return (int)sent;
        }
    }
    ...
    xqc_webtransport_stream_callbacks_t  *wt_stream_cbs)
{
    ...
    /* set WT SETTINGS on all ALPNs (RFC 9220: both peers MUST send these) */
    const char *alpns[] = { XQC_ALPN_H3, XQC_ALPN_H3_29, XQC_ALPN_H3_EXT };
    for (int a = 0; a < 3; a++) {
        xqc_h3_conn_settings_t *h3s = NULL;
        if (xqc_h3_ctx_get_default_conn_settings(engine, (char *)alpns[a],
                strlen(alpns[a]), &h3s) == XQC_OK && h3s) {
            h3s->enable_webtransport = 1;
            h3s->h3_datagram = 1;
    ...
    const void *data, size_t data_len, int fin)
{
    ...
    if (h3s == NULL) {
        h3s = xqc_h3_stream_create(h3c, stream,
                                    XQC_H3_STREAM_TYPE_WEBTRANSPORT, NULL);
        if (h3s) {
            stream->user_data = h3s;
        }
    } else {
        h3s->type = XQC_H3_STREAM_TYPE_WEBTRANSPORT;
    }
    if (h3s && h3s->h3_ext_bs == NULL) {
        h3s->h3_ext_bs = xqc_h3_ext_bytestream_create_passive(h3c, h3s, NULL);
    }

    /* build WT bidi stream header: varint(0x41) + varint(session_id) */
    uint8_t wt_hdr[16];
    unsigned char *p = wt_hdr;
    p = xqc_put_varint(p, 0x41);                /* WEBTRANSPORT_STREAM */
    p = xqc_put_varint(p, session->sessionID);   /* session_id */
