...
xqc_engine_t *test_create_engine_buf_server(test_ctx *tctx);
xqc_engine_t *test_create_engine_buf_client(test_ctx *tctx);
static xqc_int_t xqc_test_coalesced_setup(test_ctx *cli, test_ctx *svr);
static void xqc_test_coalesced_teardown(test_ctx *cli, test_ctx *svr);
    ...
    unsigned char *buf, size_t buf_cap)
{
    ...
    if (packet_out->po_used_size + 16 >= XQC_TEST_COALESCED_PACKET_SIZE) {
        goto end;
    }

    padding_size = XQC_TEST_COALESCED_PACKET_SIZE
                   - packet_out->po_used_size - 16;
