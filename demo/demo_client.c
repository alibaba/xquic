...
typedef struct xqc_demo_cli_user_conn_s {
    ...
    xqc_flag_t              trigger_cid_rotation;
...
xqc_demo_cli_close_task(xqc_demo_cli_task_t *task)
{
    ...
    if (user_conn->ev_path_cid_retirement) {
        event_del(user_conn->ev_path_cid_retirement);
        user_conn->ev_path_cid_retirement = NULL;
    }
...
{
    int ch = 0;
    while ((ch = getopt(argc, argv, "a:A:bB:c:CdD:eEf:F:G:i:I:k:K:l:L:m:MNn:op:PQr:R:s:S:t:T:U:u:V:w:Z:0")) != -1) {
        switch (ch) {
        /* server ip */
        case 'a':
