        ...
        while (node) {
            ...
            if (ps && ps->type == XQC_WT_PENDING_UNISTREAM && ps->stream) {
                    ...
                    && (uni->pending_bytes || uni->pending_fin))
                {
                    ...
                    while (buf) {
                        int processed = 0;
                        if (wt_ctx->stream_cbs.wt_unistream_read_notify) {
                            wt_ctx->stream_cbs.wt_unistream_read_notify(uni,
                                session, buf->data, buf->len, &processed);
                        }
                        buf = buf->next;
                    }
                    if (uni->pending_fin
                       ...
                       && ps->stream)
            {
                    ...
                    && (bidi->pending_bytes || bidi->pending_fin))
                {
                    ...
                    while (buf) {
                        int processed = 0;
                        if (wt_ctx->stream_cbs.wt_bidistream_read_notify) {
                            wt_ctx->stream_cbs.wt_bidistream_read_notify(bidi,
                                session, buf->data, buf->len, &processed);
                        }
                        buf = buf->next;
                    }
                ...
                if (ps_session_id == session->session_id) {
                            ...
                            stream_id, ps) == XQC_OK)
                    {
                        ...
                        if (wt_ctx && wt_ctx->stream_cbs.wt_unistream_create_notify) {
                            wt_ctx->stream_cbs.wt_unistream_create_notify(uni,
                                session, xqc_h3_conn_get_user_data(wt_conn->h3_conn));
                        }
                    }
                }
                ...
                if (ps_session_id == session->session_id) {
                            ...
                            stream_id, ps) == XQC_OK)
                    {
                        ...
                        if (wt_ctx && wt_ctx->stream_cbs.wt_bidistream_create_notify) {
                            wt_ctx->stream_cbs.wt_bidistream_create_notify(bidi,
                                session, xqc_h3_conn_get_user_data(wt_conn->h3_conn));
                        }
                    }
                }
    ...
    while (node) {
        ...
        if (node->session_id == session->session_id) {
            wt_ctx->dgram_cbs.dgram_read_notify(session, node->data,
                node->data_len, node->user_data, node->recv_time);
            if (prev) {
                prev->next = next;
            } else {
...
xqc_wt_get_ctx_by_conn(xqc_connection_t *conn)
{
    ...
    if (conn && conn->proto_data) {
        return xqc_wt_ctx_get_by_h3_alpn((xqc_h3_conn_t *)conn->proto_data);
    }
    return NULL;
}
...
{
    xqc_wt_conn_t *wt_conn = h3c_user_data
        ? (xqc_wt_conn_t *)h3c_user_data
        : (xqc_wt_conn_t *)xqc_h3_conn_get_user_data(h3_conn);
    if (wt_conn) {
        ...
        if (xqc_wt_conn_can_buffer_unknown_session(wt_conn, session_id)) {
            if (size > (size_t)nread) {
                xqc_int_t buf_ret = xqc_wt_buffer_append(
                    &wt_unistream->pending_recv, data + nread, size - nread,
    ...
    xqc_h3_stream_t *h3_stream, int *ret)
{
    ...
    if (wt_session && wt_ctx && wt_ctx->stream_cbs.wt_bidistream_create_notify) {
    ...
    uint64_t max_sessions, const char **alpns, size_t alpn_count)
{
    ...
    /* Store wt_ctx only on explicitly selected ALPN contexts. */
    for (size_t a = 0; a < selected_count; a++) {
        xqc_h3_ctx_t *h3_ctx = xqc_engine_get_alpn_ctx(engine,
            selected_alpns[a], strlen(selected_alpns[a]));
        if (h3_ctx) {
            h3_ctx->ext_ctx = wt_ctx;
            xqc_wt_engine_set_default_settings_for_alpn(engine,
                selected_alpns[a], strlen(selected_alpns[a]),
                XQC_WT_MODE_DRAFT15_STRICT,
    ...
    const char *path, const char *authority, void *user_data)
{
    ...
    xqc_wt_conn_t *wt_conn = (xqc_wt_conn_t *)xqc_h3_conn_get_user_data(h3c);
