...
{   
    ...
    ret = xqc_fec_encoder(conn, XQC_TEST_STREAM, 5);
...
xqc_test_fec_init()
{
    ...
    xqc_packet_mask_init(conn, 49, 49);
    CU_ASSERT(conn->fec_ctl->decode_matrix[0][0] == 0);
    // printf("!!!!!%d", conn->fec_ctl->decode_matrix[0][0]);
    
    xqc_packet_mask_init(conn, 48, 47);
    CU_ASSERT(conn->fec_ctl->decode_matrix[0][0] == 0);

    xqc_packet_mask_init(conn, 48, 49);
    CU_ASSERT(conn->fec_ctl->decode_matrix[0][0] == 255);

    xqc_packet_mask_init(conn, 12, 16);
    CU_ASSERT(conn->fec_ctl->decode_matrix[0][0] == 139);
