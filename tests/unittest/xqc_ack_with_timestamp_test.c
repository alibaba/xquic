...
xqc_test_ack_with_timestamp_normal()
{
    ...
    for (int i = 0; i < ack_timestamp.report_num; i++) {
        ...
        CU_ASSERT(ack_timestamp.recv_ts[i] == recv_time_test_list[test_pkt_num - 1 - i]);
    }
...
xqc_test_ack_with_timestamp_partial_writing()
{
    ...
    ret = xqc_write_ack_with_receive_timestamps_to_one_packet(conn, path, packet_out, XQC_PNS_APP_DATA, 1);
    CU_ASSERT(ret == XQC_OK);
...
void
xqc_test_ack_with_timestamp_draft_ietf_ack_frame()
{
    xqc_packet_out_t* packet_out = xqc_packet_out_create(XQC_QUIC_MAX_MSS);
    xqc_connection_t *conn = test_engine_connect();
    xqc_path_ctx_t *path = xqc_calloc(1, sizeof(xqc_path_ctx_t));
    ...
    xqc_recv_timestamps_info_t *recv_timestamp_info = xqc_recv_timestamps_info_create();
    xqc_packet_number_t pkt_num_test_list[3] = {13, 14, 15};
    xqc_usec_t recv_time_test_list[3] = {13000, 14000, 15000};
    for (int i = 0; i < 3; i++) {
        xqc_recv_timestamps_info_add_pkt(recv_timestamp_info, pkt_num_test_list[i], recv_time_test_list[i]);
        xqc_recv_record_add(&path->path_pn_ctl->ctl_recv_record[XQC_PNS_APP_DATA], pkt_num_test_list[i]);
    }
    ...
    conn->remote_settings.receive_timestamps_exponent = 0;
    conn->local_settings.max_receive_timestamps_per_ack = 30;
    conn->local_settings.receive_timestamps_exponent = 0;
    conn->conn_create_time = 0;
    ...
    CU_ASSERT(ret == XQC_OK);
    packet_out->po_used_size += ret;
    CU_ASSERT(packet_out->po_buf[0] == 0x02);
    ...
    ret = xqc_parse_ack_with_receive_timestamps_frame(packet_in, conn, &ack_info, &ack_timestamp);
    CU_ASSERT(ret == XQC_OK);
    CU_ASSERT(ack_timestamp.report_num == 3);
    CU_ASSERT(ack_timestamp.recv_ts[0] == recv_time_test_list[2]);
    CU_ASSERT(ack_timestamp.recv_ts[1] == recv_time_test_list[1]);
    CU_ASSERT(ack_timestamp.recv_ts[2] == recv_time_test_list[0]);
...
void
xqc_test_ack_with_timestamp_stops_before_next_frame()
{
    xqc_packet_out_t* packet_out = xqc_packet_out_create(XQC_QUIC_MAX_MSS);
    xqc_connection_t *conn = test_engine_connect();
    xqc_path_ctx_t *path = xqc_calloc(1, sizeof(xqc_path_ctx_t));
    ...
    xqc_recv_timestamps_info_t *recv_timestamp_info = xqc_recv_timestamps_info_create();
    xqc_recv_timestamps_info_add_pkt(recv_timestamp_info, 15, 15000);
    xqc_recv_record_add(&path->path_pn_ctl->ctl_recv_record[XQC_PNS_APP_DATA], 15);
...
void
xqc_test_ack_with_timestamp_draft_section7_ranges()
{
    xqc_packet_out_t* packet_out = xqc_packet_out_create(XQC_QUIC_MAX_MSS);
    xqc_connection_t *conn = test_engine_connect();
    xqc_path_ctx_t *path = xqc_calloc(1, sizeof(xqc_path_ctx_t));
    path->path_pn_ctl = xqc_calloc(1, sizeof(xqc_pn_ctl_t));
    path->path_send_ctl = xqc_calloc(1, sizeof(xqc_send_ctl_t));
    path->path_send_ctl->ctl_largest_recv_time[XQC_PNS_APP_DATA] = 380;
    xqc_init_list_head(&path->path_pn_ctl->ctl_recv_record[XQC_PNS_APP_DATA].list_head);

    xqc_recv_timestamps_info_t *recv_timestamp_info = xqc_recv_timestamps_info_create();
    xqc_packet_number_t pkt_nums[10] = {87, 88, 89, 90, 91, 96, 97, 98, 99, 100};
    xqc_usec_t recv_times[10] = {300, 305, 310, 320, 330, 350, 355, 360, 370, 380};
    for (int i = 0; i < 10; i++) {
        xqc_recv_timestamps_info_add_pkt(recv_timestamp_info, pkt_nums[i], recv_times[i]);
        xqc_recv_record_add(&path->path_pn_ctl->ctl_recv_record[XQC_PNS_APP_DATA], pkt_nums[i]);
    }
    ...
    ret = xqc_parse_ack_with_receive_timestamps_frame(packet_in, conn, &ack_info, &ack_timestamp);
    CU_ASSERT(ret == XQC_OK);
    CU_ASSERT(ack_timestamp.report_num == 10);
    for (int i = 0; i < 10; i++) {
        CU_ASSERT(ack_timestamp.pkt_nums[i] == pkt_nums[9 - i]);
        CU_ASSERT(ack_timestamp.recv_ts[i] == recv_times[9 - i]);
    }
...
void
xqc_test_ack_with_timestamp_high_range_flag()
{
    xqc_packet_out_t* packet_out = xqc_packet_out_create(XQC_QUIC_MAX_MSS);
    xqc_connection_t *conn = test_engine_connect();
    xqc_path_ctx_t *path = xqc_calloc(1, sizeof(xqc_path_ctx_t));
    path->path_pn_ctl = xqc_calloc(1, sizeof(xqc_pn_ctl_t));
    path->path_send_ctl = xqc_calloc(1, sizeof(xqc_send_ctl_t));
    path->path_send_ctl->ctl_largest_recv_time[XQC_PNS_APP_DATA] = 36000;
    xqc_init_list_head(&path->path_pn_ctl->ctl_recv_record[XQC_PNS_APP_DATA].list_head);

    xqc_recv_timestamps_info_t *recv_timestamp_info = xqc_recv_timestamps_info_create();
    xqc_packet_number_t pkt_nums[34];
    xqc_usec_t recv_times[34];
    for (int i = 0; i < 33; i++) {
        pkt_nums[i] = i + 1;
        recv_times[i] = (i + 1) * 1000;
    }
    pkt_nums[33] = 36;
    recv_times[33] = 36000;
    ...
    conn->conn_settings.max_receive_timestamps_per_ack = 40;
    conn->conn_settings.receive_timestamps_exponent = 0;
    conn->remote_settings.max_receive_timestamps_per_ack = 40;
    conn->remote_settings.receive_timestamps_exponent = 0;
    conn->local_settings.max_receive_timestamps_per_ack = 40;
    conn->local_settings.receive_timestamps_exponent = 0;
    conn->conn_create_time = 0;
    ...
    ret = xqc_parse_ack_with_receive_timestamps_frame(packet_in, conn, &ack_info, &ack_timestamp);
    CU_ASSERT(ret == XQC_OK);
    CU_ASSERT(ack_timestamp.report_num == 34);
    CU_ASSERT(ack_timestamp.pkt_nums[0] == 36);
    CU_ASSERT(ack_timestamp.recv_ts[0] == 36000);
    CU_ASSERT(ack_timestamp.pkt_nums[1] == 33);
    CU_ASSERT(ack_timestamp.recv_ts[1] == 33000);
