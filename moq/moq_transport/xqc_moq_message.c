...
static xqc_moq_msg_func_map_t moq_msg_func_map[] = {
    {XQC_MOQ_MSG_SETUP,                xqc_moq_msg_create_setup,            xqc_moq_msg_free_setup           },
...
//    {XQC_MOQ_MSG_OBJECT_DATAGRAM,      NULL,                                NULL                             },
    ...
    {XQC_MOQ_MSG_REQUEST_OK,           xqc_moq_msg_create_request_ok,       xqc_moq_msg_free_request_ok      },
...
typedef enum {
    XQC_MOQ_PARAM_ENCODING_VI64,
    XQC_MOQ_PARAM_ENCODING_U8,
    XQC_MOQ_PARAM_ENCODING_LOCATION,
    XQC_MOQ_PARAM_ENCODING_BYTES,
} xqc_moq_param_encoding_t;

static xqc_int_t
xqc_moq_msg_param_encoding_draft18(uint64_t type,
    xqc_moq_param_encoding_t *encoding)
{
    switch (type) {
    case 0x02: /* OBJECT_DELIVERY_TIMEOUT */
    case 0x04: /* RENDEZVOUS_TIMEOUT */
    case 0x06: /* SUBGROUP_DELIVERY_TIMEOUT */
    case 0x08: /* EXPIRES */
    case 0x0A: /* FILL_TIMEOUT */
    case 0x32: /* NEW_GROUP_REQUEST */
        *encoding = XQC_MOQ_PARAM_ENCODING_VI64;
        return XQC_OK;
    case 0x10: /* FORWARD */
    case 0x20: /* SUBSCRIBER_PRIORITY */
    case 0x22: /* GROUP_ORDER */
        *encoding = XQC_MOQ_PARAM_ENCODING_U8;
        return XQC_OK;
    case 0x09: /* LARGEST_OBJECT */
        *encoding = XQC_MOQ_PARAM_ENCODING_LOCATION;
        return XQC_OK;
    case XQC_MOQ_PARAM_AUTHORIZATION_TOKEN:
    case 0x21: /* SUBSCRIPTION_FILTER */
        *encoding = XQC_MOQ_PARAM_ENCODING_BYTES;
        return XQC_OK;
    default:
        return -XQC_EILLEGAL_FRAME;
    }
    ...
    xqc_moq_message_parameter_t *params, uint64_t params_num)
{
    ...
    for (uint64_t i = 0; i < params_num; i++) {
        ...
        xqc_int_t ret = xqc_vi64_read(*pos, end, &delta_type);
        if (ret < 0 || UINT64_MAX - previous_type < delta_type) {
            return -XQC_EILLEGAL_FRAME;
        }
        *pos += ret;
        uint64_t type = previous_type + delta_type;
        if (i > 0 && delta_type == 0
            && type != XQC_MOQ_PARAM_AUTHORIZATION_TOKEN)
        {
            return -XQC_EILLEGAL_FRAME;
        }

        xqc_moq_param_encoding_t encoding;
        ret = xqc_moq_msg_param_encoding_draft18(type, &encoding);
        if (ret != XQC_OK) {
            return ret;
        }
        params[i].type = type;

        if (encoding == XQC_MOQ_PARAM_ENCODING_U8) {
            if (*pos >= end) {
                return -XQC_EILLEGAL_FRAME;
            }
            params[i].is_integer = 1;
            params[i].int_value = **pos;
            (*pos)++;

        } else if (encoding == XQC_MOQ_PARAM_ENCODING_VI64) {
            uint64_t value = 0;
            ret = xqc_vi64_read(*pos, end, &value);
            if (ret < 0) {
                return -XQC_EILLEGAL_FRAME;
            }
            *pos += ret;
            params[i].is_integer = 1;
            params[i].int_value = value;

        } else if (encoding == XQC_MOQ_PARAM_ENCODING_LOCATION) {
            uint8_t *value_start = *pos;
            uint64_t group = 0, object = 0;
            ret = xqc_vi64_read(*pos, end, &group);
            if (ret < 0) {
                return -XQC_EILLEGAL_FRAME;
            }
            *pos += ret;
            ret = xqc_vi64_read(*pos, end, &object);
            if (ret < 0) {
                return -XQC_EILLEGAL_FRAME;
            }
            *pos += ret;
            params[i].length = *pos - value_start;
            params[i].value = xqc_malloc(params[i].length);
            if (params[i].value == NULL) {
                return -XQC_EMALLOC;
            }
            xqc_memcpy(params[i].value, value_start, params[i].length);

        } else {
            uint64_t value_len = 0;
            ret = xqc_vi64_read(*pos, end, &value_len);
            if (ret < 0 || value_len > XQC_MOQ_MAX_PARAM_VALUE_LEN
                || (uint64_t)(end - *pos - ret) < value_len)
            {
                return -XQC_EILLEGAL_FRAME;
            }
            *pos += ret;
            params[i].length = value_len;
            if (value_len > 0) {
                params[i].value = xqc_malloc(value_len);
                if (params[i].value == NULL) {
                    return -XQC_EMALLOC;
                }
                xqc_memcpy(params[i].value, *pos, value_len);
                *pos += value_len;
            }
        }
        previous_type = type;
...
static xqc_int_t
xqc_moq_msg_validate_track_properties_draft18(uint8_t *pos, uint8_t *end)
{
    uint64_t previous_type = 0;
    while (pos < end) {
        uint64_t delta_type = 0;
    ...
    return pos == end ? XQC_OK : -XQC_EILLEGAL_FRAME;
}
...
static xqc_int_t
xqc_moq_msg_validate_setup_options(const uint8_t *options, size_t options_len)
{
    const uint8_t *pos = options;
    ...
    while (pos < end) {
        ...
        uint64_t value_or_len = 0;
        ret = xqc_vi64_read(pos, end, &value_or_len);
        if (ret < 0) {
            return -XQC_EILLEGAL_FRAME;
        }
        pos += ret;
        if (type & 1) {
            if (value_or_len > UINT16_MAX || (uint64_t)(end - pos) < value_or_len) {
                return -XQC_EILLEGAL_FRAME;
            }
            pos += value_or_len;
        }
    ...
    xqc_int_t *finish, xqc_int_t *wait_more_data)
{
    ...
    *wait_more_data = 0;
    ...
    if (setup->options_processed == setup->options_len) {
        if (xqc_moq_msg_validate_setup_options(setup->options, setup->options_len) != XQC_OK) {
            return -XQC_EILLEGAL_FRAME;
        }
        *finish = 1;
    } else if (stream_fin) {
...
xqc_moq_msg_encode_subscribe_len(xqc_moq_msg_base_t *msg_base)
{
    ...
    len += xqc_moq_track_namespace_tuple_encode_len(subscribe->track_namespace_num, subscribe->track_namespace_tuple);
...
xqc_moq_msg_encode_subscribe(xqc_moq_msg_base_t *msg_base, uint8_t *buf, size_t buf_cap)
{
    ...
    p = xqc_moq_track_namespace_tuple_encode(p, subscribe->track_namespace_num, subscribe->track_namespace_tuple);
...
xqc_moq_msg_encode_subscribe_request_len(xqc_moq_msg_base_t *msg_base)
{
    ...
    if (subscribe->params_num != 0
        || subscribe->track_name_len > XQC_MOQ_MAX_NAME_LEN
    ...
    xqc_int_t payload_len = xqc_vi64_len(subscribe->subscribe_id)
        ...
        + xqc_vi64_len(subscribe->params_num);
    ...
    xqc_int_t *wait_more_data)
{
    ...
    *wait_more_data = 0;
    ...
    if (subscribe->params_num > 0) {
        ...
        ret = xqc_moq_msg_decode_params_draft18(&p, end, subscribe->params,
                                                subscribe->params_num);
        if (ret != XQC_OK) {
            return ret;
        }
    }
    ...
    xqc_moq_msg_base_t *msg_base, xqc_int_t *finish, xqc_int_t *wait_more_data)
{
    ...
    *wait_more_data = 0;
    ...
    xqc_int_t ret = 0;
    xqc_int_t param_finish = 0;
    xqc_int_t payload_offset = 0;
    xqc_moq_subscribe_msg_t *subscribe = (xqc_moq_subscribe_msg_t *)msg_base;
    xqc_moq_decode_params_ctx_t *params_ctx = &msg_ctx->decode_params_ctx;

    size_t msg_bound_len = buf_len;
    if (msg_ctx->cur_field_idx > 0) {
        uint64_t remaining = msg_ctx->msg_declared_length - msg_ctx->msg_payload_consumed;
        msg_bound_len = remaining < buf_len ? (size_t)remaining : buf_len;
    }

    switch (msg_ctx->cur_field_idx) {
        case 0: // Length
            ret = xqc_moq_length_read(buf + processed, buf + buf_len, &msg_ctx->msg_declared_length);
            if (ret < 0) {
                *wait_more_data = 1;
                break;
            }
            processed += ret;
            payload_offset = processed;
            msg_ctx->msg_payload_consumed = 0;
            {
                uint64_t end = (uint64_t)payload_offset + msg_ctx->msg_declared_length;
                msg_bound_len = end < buf_len ? (size_t)end : buf_len;
            }
            msg_ctx->cur_field_idx = 1;
        case 1: //Subscribe ID (i)
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &subscribe->subscribe_id);
            if (ret < 0) {
        ...
        case 2: //Track Namespace tuple count
            if (subscribe->track_namespace_num == 0) {
                ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &subscribe->track_namespace_num);
                if (ret < 0) {
                    if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                    *wait_more_data = 1;
                    break;
            ...
            msg_ctx->cur_field_idx = 3;
        case 3: //Track Namespace tuple elements
            for (; msg_ctx->cur_array_idx < subscribe->track_namespace_num; msg_ctx->cur_array_idx++) {
                xqc_moq_track_ns_field_t *field = &subscribe->track_namespace_tuple[msg_ctx->cur_array_idx];
                if (!msg_ctx->tuple_elem_len_ready) {
                    ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &msg_ctx->tuple_elem_len);
                    if (ret < 0) {
                        if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                        *wait_more_data = 1; break;
                    }
                    processed += ret;
                ...
                size_t avail = msg_bound_len - processed;
                if (field_remaining <= avail) {
                    xqc_memcpy(field->data + msg_ctx->str_processed, buf + processed, field_remaining);
                    processed += field_remaining;
                } else {
                    xqc_memcpy(field->data + msg_ctx->str_processed, buf + processed, avail);
                    msg_ctx->str_processed += avail;
                    processed += avail;
                    if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                    *wait_more_data = 1;
                    break;
        ...
        case 4: //Track Name (b)
            ...
            {
                size_t name_remaining = subscribe->track_name_len - msg_ctx->str_processed;
                size_t avail = msg_bound_len - processed;
                if (name_remaining <= avail) {
                    xqc_memcpy(subscribe->track_name + msg_ctx->str_processed, buf + processed, name_remaining);
                    processed += name_remaining;
                    msg_ctx->str_processed = 0;
                } else {
                    xqc_memcpy(subscribe->track_name + msg_ctx->str_processed, buf + processed, avail);
                    msg_ctx->str_processed += avail;
                    processed += avail;
                    if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                    *wait_more_data = 1;
        ...
        case 6: //Group order (8)
            ret = xqc_moq_read_u8(buf, msg_bound_len, &processed, &subscribe->group_order);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
        ...
        case 7: //Forward (8)
            ret = xqc_moq_read_u8(buf, msg_bound_len, &processed, &subscribe->forward);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
        ...
        case 8: //Filter Type (i)
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &subscribe->filter_type);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
        ...
        case 9: //StartGroup (i)
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &subscribe->start_group_id);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
        ...
        case 10: //StartObject (i)
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &subscribe->start_object_id);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
        ...
        case 11: //EndGroup (i)
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &subscribe->end_group_id);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
        ...
        case 12: //EndObject (i)
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &subscribe->end_object_id);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
        ...
        idx_params:
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &subscribe->params_num);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
        ...
        case 14: //Subscribe Parameters (..) ...
        {
            ...
            processed += ret;
            if (*wait_more_data == 1) {
                if (msg_bound_len < buf_len && !param_finish) { return -MOQ_PROTOCOL_VIOLATION; }
                break;
            }
            if (!param_finish) {
                break;
        ...
        trailing_skip:
        {
            ...
            payload_offset = processed;
            uint64_t trailing = msg_ctx->msg_declared_length - msg_ctx->msg_payload_consumed;
            if (trailing > 0) {
                uint64_t buf_avail = buf_len - processed;
                if (trailing <= buf_avail) {
                    processed += trailing;
                } else {
                    processed += buf_avail;
                    msg_ctx->msg_payload_consumed += buf_avail;
                    *wait_more_data = 1;
                    break;
                }
            }
    ...
    if (!*finish && msg_ctx->cur_field_idx > 0 && msg_ctx->cur_field_idx < 15) {
        msg_ctx->msg_payload_consumed += (processed - payload_offset);
    }
    return processed;
}
...
xqc_moq_msg_encode_subscribe_ok_response_len(xqc_moq_msg_base_t *msg_base)
{
    ...
    if (subscribe_ok->params_num != 0
        || (subscribe_ok->track_properties_len > 0
            && (subscribe_ok->track_properties == NULL
    ...
    xqc_int_t payload_len = xqc_vi64_len(subscribe_ok->track_alias)
    ...
    xqc_int_t *wait_more_data)
{
    ...
    *wait_more_data = 0;
    ...
    if (subscribe_ok->params_num > 0) {
        ...
        ret = xqc_moq_msg_decode_params_draft18(&p, end, subscribe_ok->params,
                                                subscribe_ok->params_num);
        if (ret != XQC_OK) {
            return ret;
        }
    }

    ret = xqc_moq_msg_validate_track_properties_draft18(p, end);
    if (ret != XQC_OK) {
        return ret;
    }
...
xqc_moq_msg_free_publish(void *msg)
{
    ...
    xqc_moq_namespace_tuple_free(publish->track_namespace_tuple, publish->track_namespace_num);
...
xqc_moq_msg_encode_publish_len(xqc_moq_msg_base_t *msg_base)
{
    ...
    len += xqc_moq_track_namespace_tuple_encode_len(publish->track_namespace_num, publish->track_namespace_tuple);
...
xqc_moq_msg_encode_publish(xqc_moq_msg_base_t *msg_base, uint8_t *buf, size_t buf_cap)
{
    ...
    p = xqc_moq_track_namespace_tuple_encode(p, publish->track_namespace_num, publish->track_namespace_tuple);
    ...
    *wait_more_data = 0;
    ...
    switch (msg_ctx->cur_field_idx) {
        case 0: // length
            ...
            msg_ctx->cur_field_idx = 1;
        case 1: //Request ID (i)
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &publish->subscribe_id);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
        ...
        case 2: //Track Namespace tuple count
            if (publish->track_namespace_num == 0) {
                ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &publish->track_namespace_num);
                if (ret < 0) {
                    if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                    *wait_more_data = 1;
                    break;
                ...
                processed += ret;
                if (publish->track_namespace_num == 0
                    || publish->track_namespace_num > XQC_MOQ_MAX_NAMESPACE_TUPLE_ELEMS)
                {
                    return -MOQ_PROTOCOL_VIOLATION;
                }
                msg_ctx->cur_array_idx = 0;
        ...
        case 3: //Track Namespace tuple elements
            for (; msg_ctx->cur_array_idx < publish->track_namespace_num; msg_ctx->cur_array_idx++) {
                xqc_moq_track_ns_field_t *field = &publish->track_namespace_tuple[msg_ctx->cur_array_idx];
                if (!msg_ctx->tuple_elem_len_ready) {
                    ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &msg_ctx->tuple_elem_len);
                    if (ret < 0) {
                        if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
        ...
        case 4: //Track Name (b)
            if (publish->track_name_len == 0) {
                ret = xqc_vint_read(buf + processed, buf + msg_bound_len, (uint64_t*)&publish->track_name_len);
                if (ret < 0) {
                    if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                    *wait_more_data = 1;
                    break;
            ...
            {
                size_t name_remaining = publish->track_name_len - msg_ctx->str_processed;
                size_t avail = msg_bound_len - processed;
                if (name_remaining <= avail) {
                    xqc_memcpy(publish->track_name + msg_ctx->str_processed, buf + processed, name_remaining);
                    processed += name_remaining;
                    msg_ctx->str_processed = 0;
                } else {
                    xqc_memcpy(publish->track_name + msg_ctx->str_processed, buf + processed, avail);
                    msg_ctx->str_processed += avail;
                    processed += avail;
                    if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                    *wait_more_data = 1;
        ...
        case 5: //Track Alias (i)
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &publish->track_alias);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
            ...
            msg_ctx->cur_field_idx = 6;
        case 6: //Group Order (8)
            ret = xqc_moq_read_u8(buf, msg_bound_len, &processed, &publish->group_order);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
            ...
            msg_ctx->cur_field_idx = 7;
        case 7: //Content Exists (8)
            ret = xqc_moq_read_u8(buf, msg_bound_len, &processed, &publish->content_exist);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
        ...
        case 8: //Largest Group ID (i)
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &publish->largest_group_id);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
            ...
            processed += ret;
            msg_ctx->cur_field_idx = 9;
        case 9: //Largest Object ID (i)
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &publish->largest_object_id);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
        ...
        case 10: //Forward (8)
            idx10:
            ret = xqc_moq_read_u8(buf, msg_bound_len, &processed, &publish->forward);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
            ...
            msg_ctx->cur_field_idx = 11;
        case 11: //Number of Parameters (i)
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &publish->params_num);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
                break;
        ...
        {
            size_t params_avail = msg_bound_len > (size_t)processed ? msg_bound_len - processed : 0;
            ret = xqc_moq_msg_decode_params_v14(buf + processed, params_avail, params_ctx,
    ...
    if (!*finish && msg_ctx->cur_field_idx > 0 && msg_ctx->cur_field_idx < 13) {
        msg_ctx->msg_payload_consumed += (processed - payload_offset);
    }
    return processed;
}
...
xqc_moq_msg_free_publish_namespace(void *msg)
{
    ...
    xqc_moq_publish_namespace_msg_t *pub = (xqc_moq_publish_namespace_msg_t*)msg;
    if (pub->track_namespace_tuple) {
        for (uint64_t i = 0; i < pub->track_namespace_num; i++) {
            xqc_free(pub->track_namespace_tuple[i].data);
        }
...
xqc_moq_msg_encode_publish_namespace_len_vi64(xqc_moq_msg_base_t *msg_base)
{
    ...
    size_t track_namespace_len = 0;
    if (pub->params_num != 0
        || xqc_moq_track_namespace_tuple_validate_and_sum(pub->track_namespace_num,
            pub->track_namespace_tuple, &track_namespace_len) != XQC_OK)
    {
        return -XQC_EPARAM;
    }

    xqc_int_t payload_len = xqc_vi64_len(pub->request_id)
        + xqc_moq_track_namespace_tuple_encode_len_vi64(pub->track_namespace_num,
            pub->track_namespace_tuple)
        + xqc_vi64_len(pub->params_num);
    if (payload_len > UINT16_MAX) {
        return -XQC_ELIMIT;
    }
...
xqc_moq_msg_encode_publish_namespace_len(xqc_moq_msg_base_t *msg_base)
{
    xqc_moq_publish_namespace_msg_t *pub = (xqc_moq_publish_namespace_msg_t*)msg_base;
    size_t track_namespace_len = 0;
    if (xqc_moq_track_namespace_tuple_validate_and_sum(pub->track_namespace_num,
            pub->track_namespace_tuple, &track_namespace_len) != XQC_OK)
    { return -XQC_EPARAM; }
    ...
    len += XQC_MOQ_MSG_LENGTH_FIXED_SIZE;
    len += xqc_put_varint_len(pub->request_id);
    len += xqc_moq_track_namespace_tuple_encode_len(pub->track_namespace_num, pub->track_namespace_tuple);
    len += xqc_put_varint_len(pub->params_num);
    len += xqc_moq_msg_encode_params_len_v14(pub->params, pub->params_num);
    return len;
}
...
xqc_moq_msg_encode_publish_namespace(xqc_moq_msg_base_t *msg_base, uint8_t *buf, size_t buf_cap)
{
    ...
    p = xqc_moq_put_varint_length(p, length);
    p = xqc_put_varint(p, pub->request_id);
    p = xqc_moq_track_namespace_tuple_encode(p, pub->track_namespace_num, pub->track_namespace_tuple);
    p = xqc_put_varint(p, pub->params_num);
    xqc_int_t ret = xqc_moq_msg_encode_params_v14(pub->params, pub->params_num, p, buf + buf_cap - p);
    if (ret < 0) { return ret; }
    p += ret;
    return p - buf;
}
    ...
    xqc_int_t *finish, xqc_int_t *wait_more_data)
{
    *finish = 0; *wait_more_data = 0;
    xqc_int_t processed = 0, ret = 0, param_finish = 0;
    xqc_int_t payload_offset = 0;
    xqc_moq_publish_namespace_msg_t *pub = (xqc_moq_publish_namespace_msg_t*)msg_base;
    xqc_moq_decode_params_ctx_t *params_ctx = &msg_ctx->decode_params_ctx;
    ...
    switch (msg_ctx->cur_field_idx) {
        case 0:
            ...
            msg_ctx->cur_field_idx = 1;
        case 1:
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &pub->request_id);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1; break;
            }
            ...
            msg_ctx->cur_field_idx = 2;
        case 2:
            if (pub->track_namespace_num == 0) {
                ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &pub->track_namespace_num);
                if (ret < 0) {
                    if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                    *wait_more_data = 1; break;
                }
                processed += ret;
                if (pub->track_namespace_num == 0 || pub->track_namespace_num > XQC_MOQ_MAX_NAMESPACE_TUPLE_ELEMS) {
                    return -MOQ_PROTOCOL_VIOLATION;
                }
                msg_ctx->cur_array_idx = 0;
                msg_ctx->str_processed = 0;
                ...
                msg_ctx->tuple_elem_len_ready = 0;
                msg_ctx->namespace_total_len = 0;
                pub->track_namespace_tuple = xqc_calloc(pub->track_namespace_num, sizeof(xqc_moq_track_ns_field_t));
                if (pub->track_namespace_tuple == NULL) { return -XQC_EMALLOC; }
            }
            msg_ctx->cur_field_idx = 3;
        case 3:
            for (; msg_ctx->cur_array_idx < pub->track_namespace_num; msg_ctx->cur_array_idx++) {
                xqc_moq_track_ns_field_t *field = &pub->track_namespace_tuple[msg_ctx->cur_array_idx];
                if (!msg_ctx->tuple_elem_len_ready) {
                    ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &msg_ctx->tuple_elem_len);
                    if (ret < 0) {
                        if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
        ...
        case 4:
            if (pub->params_num == 0) {
                ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &pub->params_num);
                if (ret < 0) {
                    if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                    *wait_more_data = 1; break;
                }
                ...
                if (pub->params_num > XQC_MOQ_MAX_PARAMS) { return -XQC_ELIMIT; }
                pub->params = xqc_moq_msg_alloc_params(pub->params_num);
                if (pub->params == NULL) { return -XQC_EMALLOC; }
            }
            msg_ctx->cur_field_idx = 5;
        case 5:
        {
            size_t params_avail = msg_bound_len > (size_t)processed ? msg_bound_len - processed : 0;
            ret = xqc_moq_msg_decode_params_v14(buf + processed, params_avail, params_ctx,
                                                pub->params, pub->params_num, &param_finish, wait_more_data);
            if (ret < 0) { return ret; }
            processed += ret;
            if (*wait_more_data) {
                if (msg_bound_len < buf_len && !param_finish) { return -MOQ_PROTOCOL_VIOLATION; }
    ...
    if (!*finish && msg_ctx->cur_field_idx > 0 && msg_ctx->cur_field_idx < 6) {
        msg_ctx->msg_payload_consumed += (processed - payload_offset);
    }
    return processed;
}
...
xqc_moq_msg_encode_request_ok_len(xqc_moq_msg_base_t *msg_base)
{
    ...
    return xqc_vi64_len(XQC_MOQ_MSG_REQUEST_OK)
        + XQC_MOQ_MSG_LENGTH_FIXED_SIZE + xqc_vi64_len(msg->params_num);
}
    ...
    size_t buf_cap)
{
    ...
    p = xqc_moq_put_varint_length(p, xqc_vi64_len(msg->params_num));
    ...
    xqc_int_t *finish, xqc_int_t *wait_more_data)
{
    ...
    *wait_more_data = 0;
    ...
    xqc_int_t ret = xqc_vi64_read(buf + processed, buf + buf_len, &msg->params_num);
    if (ret < 0) {
        if (stream_fin) {
            return -XQC_EILLEGAL_FRAME;
        }
    ...
    /* Parameter parsing will be shared with other request messages as they are ported.
     * PUBLISH_NAMESPACE_OK has no parameters or Track Properties. */
    if (msg->params_num != 0 || (uint64_t)ret != msg_ctx->msg_declared_length) {
        return -XQC_EILLEGAL_FRAME;
    }
...
xqc_moq_msg_encode_publish_namespace_done_len(xqc_moq_msg_base_t *msg_base)
{
    ...
    len += XQC_MOQ_MSG_LENGTH_FIXED_SIZE;
    len += xqc_moq_track_namespace_tuple_encode_len(done->track_namespace_num, done->track_namespace_tuple);
    return len;
}
...
xqc_moq_msg_encode_publish_namespace_done(xqc_moq_msg_base_t *msg_base, uint8_t *buf, size_t buf_cap)
{
    ...
    p = xqc_moq_put_varint_length(p, length);
    p = xqc_moq_track_namespace_tuple_encode(p, done->track_namespace_num, done->track_namespace_tuple);
    return p - buf;
}
    ...
    switch (msg_ctx->cur_field_idx) {
        ...
        case 1:
            if (done->track_namespace_num == 0) {
                ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &done->track_namespace_num);
                if (ret < 0) {
                    if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                    *wait_more_data = 1; break;
                }
                processed += ret;
                if (done->track_namespace_num == 0 || done->track_namespace_num > XQC_MOQ_MAX_NAMESPACE_TUPLE_ELEMS) {
                    return -MOQ_PROTOCOL_VIOLATION;
                }
                msg_ctx->cur_array_idx = 0;
                msg_ctx->str_processed = 0;
                ...
                if (done->track_namespace_tuple == NULL) { return -XQC_EMALLOC; }
            }
            msg_ctx->cur_field_idx = 2;
        case 2:
...
xqc_moq_msg_encode_subscribe_namespace_len(xqc_moq_msg_base_t *msg_base)
{
    ...
    len += xqc_moq_track_namespace_tuple_encode_len(sub->track_namespace_num, sub->track_namespace_tuple);
...
xqc_moq_msg_encode_subscribe_namespace(xqc_moq_msg_base_t *msg_base, uint8_t *buf, size_t buf_cap)
{
    ...
    p = xqc_moq_track_namespace_tuple_encode(p, sub->track_namespace_num, sub->track_namespace_tuple);
    p = xqc_put_varint(p, sub->params_num);
    xqc_int_t ret = xqc_moq_msg_encode_params_v14(sub->params, sub->params_num, p, buf + buf_cap - p);
    if (ret < 0) { return ret; }
    p += ret;
    return p - buf;
}
    ...
    *finish = 0; *wait_more_data = 0;
    ...
    switch (msg_ctx->cur_field_idx) {
        case 0:
            ...
            msg_ctx->cur_field_idx = 1;
        case 1:
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &sub->request_id);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1; break;
            }
        ...
        case 2:
            if (sub->track_namespace_num == 0) {
                ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &sub->track_namespace_num);
                if (ret < 0) {
                    if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                    *wait_more_data = 1; break;
                }
                processed += ret;
                if (sub->track_namespace_num == 0 || sub->track_namespace_num > XQC_MOQ_MAX_NAMESPACE_TUPLE_ELEMS) {
                    return -MOQ_PROTOCOL_VIOLATION;
                }
                msg_ctx->cur_array_idx = 0;
                msg_ctx->str_processed = 0;
        ...
        case 4:
            if (sub->params_num == 0) {
                ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &sub->params_num);
                if (ret < 0) {
                    if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                    *wait_more_data = 1; break;
                }
                ...
                if (sub->params_num > XQC_MOQ_MAX_PARAMS) { return -XQC_ELIMIT; }
                sub->params = xqc_moq_msg_alloc_params(sub->params_num);
                if (sub->params == NULL) { return -XQC_EMALLOC; }
            }
            msg_ctx->cur_field_idx = 5;
        case 5:
        {
    ...
    *finish = 0; *wait_more_data = 0;
    ...
    switch (msg_ctx->cur_field_idx) {
        case 0:
            ...
            msg_ctx->cur_field_idx = 1;
        case 1:
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &ok->request_id);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1; break;
            }
        ...
        case 2:
        {
            ...
            uint64_t trailing = msg_ctx->msg_declared_length - msg_ctx->msg_payload_consumed;
            if (trailing > 0) {
                uint64_t avail = buf_len - processed;
                if (trailing <= avail) {
                    processed += trailing;
                } else {
                    processed += avail;
                    msg_ctx->msg_payload_consumed += avail;
                    *wait_more_data = 1;
                    break;
                }
            }
    ...
    if (!*finish && msg_ctx->cur_field_idx > 0 && msg_ctx->cur_field_idx < 2) {
        msg_ctx->msg_payload_consumed += (processed - payload_offset);
    }
    return processed;
}
    ...
    *finish = 0; *wait_more_data = 0;
    ...
    switch (msg_ctx->cur_field_idx) {
        case 0:
            ...
            msg_ctx->cur_field_idx = 1;
        case 1:
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &err->request_id);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1; break;
            }
            ...
            msg_ctx->cur_field_idx = 2;
        case 2:
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &err->error_code);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1; break;
            }
        ...
        case 3:
        {
            ...
            ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &reason_len);
            if (ret < 0) {
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1; break;
            }
            processed += ret;
            err->reason_phrase_len = reason_len;
            if (reason_len > XQC_MOQ_MAX_NAME_LEN) { return -XQC_ELIMIT; }
            if (reason_len > 0) {
                err->reason_phrase = xqc_calloc(1, reason_len + 1);
                if (err->reason_phrase == NULL) { return -XQC_EMALLOC; }
            }
            if (reason_len == 0) {
                msg_ctx->cur_field_idx = 5;
                goto trailing_skip;
        ...
        case 4:
        {
            ...
            size_t str_remaining = err->reason_phrase_len - msg_ctx->str_processed;
            size_t avail = msg_bound_len - processed;
            if (str_remaining <= avail) {
                xqc_memcpy(err->reason_phrase + msg_ctx->str_processed, buf + processed, str_remaining);
                processed += str_remaining;
            } else {
                xqc_memcpy(err->reason_phrase + msg_ctx->str_processed, buf + processed, avail);
                msg_ctx->str_processed += avail;
                processed += avail;
                if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                *wait_more_data = 1;
    ...
    if (!*finish && msg_ctx->cur_field_idx > 0 && msg_ctx->cur_field_idx < 5) {
        msg_ctx->msg_payload_consumed += (processed - payload_offset);
    }
    return processed;
}
...
xqc_moq_msg_encode_unsubscribe_namespace_len(xqc_moq_msg_base_t *msg_base)
{
    ...
    len += XQC_MOQ_MSG_LENGTH_FIXED_SIZE;
    len += xqc_moq_track_namespace_tuple_encode_len(unsub->track_namespace_num, unsub->track_namespace_tuple);
    return len;
}
...
xqc_moq_msg_encode_unsubscribe_namespace(xqc_moq_msg_base_t *msg_base, uint8_t *buf, size_t buf_cap)
{
    ...
    p = xqc_moq_put_varint_length(p, length);
    p = xqc_moq_track_namespace_tuple_encode(p, unsub->track_namespace_num, unsub->track_namespace_tuple);
    return p - buf;
}
    ...
    switch (msg_ctx->cur_field_idx) {
        ...
        case 1:
            if (unsub->track_namespace_num == 0) {
                ret = xqc_vint_read(buf + processed, buf + msg_bound_len, &unsub->track_namespace_num);
                if (ret < 0) {
                    if (msg_bound_len < buf_len) { return -MOQ_PROTOCOL_VIOLATION; }
                    *wait_more_data = 1; break;
                }
                processed += ret;
                if (unsub->track_namespace_num == 0 || unsub->track_namespace_num > XQC_MOQ_MAX_NAMESPACE_TUPLE_ELEMS) {
                    return -MOQ_PROTOCOL_VIOLATION;
                }
                msg_ctx->cur_array_idx = 0;
                msg_ctx->str_processed = 0;
