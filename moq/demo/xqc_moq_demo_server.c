...
xqc_server_get_moq_callbacks(void)
{
...
xqc_server_accept(xqc_engine_t *engine, xqc_connection_t *conn, const xqc_cid_t *cid, void *user_data)
{
    ...
    if (!g_use_wt) {
        /* for WT, moq session is created later, in webtransport_session_create_notify */
        xqc_moq_session_callbacks_t callbacks = xqc_server_get_moq_callbacks();
        xqc_moq_session_t *session = xqc_moq_session_create(conn, user_session,
...
int
xqc_server_h3_conn_close_notify(xqc_h3_conn_t *h3_conn, const xqc_cid_t *cid, void *h3c_user_data)
{
    DEBUG;
    ...
    xqc_http_headers_t *headers, const xqc_cid_t *cid, void *h3c_user_data)
{
    ...
    xqc_moq_session_t *session = xqc_moq_session_create(wt_session, user_session,
        XQC_MOQ_TRANSPORT_WEBTRANSPORT, g_role, callbacks, NULL);
    if (session == NULL) {
...
int
xqc_server_wt_session_close_notify(xqc_webtransport_session_t *wt_session,
    xqc_http_headers_t *headers, const xqc_cid_t *cid, void *h3c_user_data)
{
    DEBUG;
    xqc_moq_user_session_t *user_session = (xqc_moq_user_session_t *)h3c_user_data;
     ...
     * chain.  Destroying the moq session synchronously here would free
     * memory the outer chain still reads (use-after-free).  Only mark the
...
int main(int argc, char *argv[])
{
    ...
    while ((ch = getopt(argc, argv, "p:r:c:l:n:fd:w")) != -1) {
        switch (ch) {
        /* listen port */
        case 'p':
    ...
    if (g_use_wt) {
        ...
        int wt_ret = xqc_h3_ctx_init(ctx.engine, &h3_cbs);
        if (wt_ret != XQC_OK) {
        ...
        wt_ret = xqc_wt_ctx_init_for_alpns(ctx.engine, &wt_dgram_cbs, &wt_session_cbs,
