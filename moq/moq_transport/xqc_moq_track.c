    ...
    xqc_moq_track_type_t track_type, xqc_moq_selection_params_t *params, xqc_moq_container_t container, xqc_moq_track_role_t role)
{
    ...
    switch (track_type) {
        ...
        case XQC_MOQ_TRACK_CATALOG:
            ...
            break;
        case XQC_MOQ_TRACK_DEFAULT:
            track = xqc_calloc(1, sizeof(xqc_moq_default_track_t));
    ...
    track->track_alias = XQC_MOQ_INVALID_ALIAS;
    track->subscribe_id = XQC_MOQ_INVALID_ID;

    if (role == XQC_MOQ_TRACK_FOR_PUB) {
        list = &session->track_list_for_pub;
        /* Auto-assign alias for PUB tracks */
        track->track_alias = xqc_moq_session_alloc_track_alias(session);
    } else {
...
xqc_moq_track_destroy(xqc_moq_track_t *track)
{
    ...
    xqc_moq_namespace_notify_on_track_removed(track->session, track);
...
xqc_int_t
xqc_moq_track_status(xqc_moq_session_t *session, xqc_moq_track_status_msg_t *track_status)
{
    xqc_int_t ret = 0;
    ret = xqc_moq_write_track_status_msg(session, track_status);
    if(ret < 0) {
        xqc_log(session->log, XQC_LOG_ERROR, "|write track status error|");
    }
    return ret;
}

xqc_int_t
xqc_moq_track_status_ok(xqc_moq_session_t *session, xqc_moq_track_status_ok_msg_t *track_status_ok)
{
    xqc_int_t ret = 0;
    ret = xqc_moq_write_track_status_ok_msg(session, track_status_ok);
    if(ret < 0) {
        xqc_log(session->log, XQC_LOG_ERROR, "|write track status ok error|");
    }
    return ret;
}

xqc_int_t
xqc_moq_track_status_error(xqc_moq_session_t *session, xqc_moq_track_status_error_msg_t *track_status_error)
{
    xqc_int_t ret = 0;
    ret = xqc_moq_write_track_status_error_msg(session, track_status_error);
    if(ret < 0) {
        xqc_log(session->log, XQC_LOG_ERROR, "|write track status error error|");
    }
    return ret;
}