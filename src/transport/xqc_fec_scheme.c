...
xqc_int_t
xqc_fec_encoder(xqc_connection_t *conn, unsigned char *input, size_t st_size)
{
    size_t tmp_size;
    xqc_int_t i, ret, symbol_idx, repair_symbol_num;
    ...
    ret = xqc_fec_encoder_check_params(conn, conn->fec_ctl->fec_send_required_repair_num, conn->conn_settings.fec_params.fec_encoder_scheme, st_size);
    if (ret != XQC_OK) {
        return ret;
    }
    ...
    if (conn->conn_settings.fec_callback.xqc_fec_encode) {
        ...
        ret = conn->conn_settings.fec_callback.xqc_fec_encode(conn, input, st_size, repair_symbols_payload_buff);
        // set validation and payload size of fec_send_repair_symbols_buff object
        for (i = 0; i < repair_symbol_num; i++) {
            tmp_size = xqc_max(st_size, conn->fec_ctl->fec_send_repair_symbols_buff[i].payload_size);
...
xqc_fec_cc_decoder(xqc_connection_t *conn, xqc_fec_rpr_syb_t *rpr_symbol, uint8_t lack_syb_id)
{
    ...
    // xqc_log(conn->log, XQC_LOG_DEBUG, "[test]|process recovered|bid:%d|sid:%d|rcv_mask:%d|symbol_size:%d", block_id, symbol_idx, rpr_symbol->recv_mask[0], rpr_symbol->payload_size);
