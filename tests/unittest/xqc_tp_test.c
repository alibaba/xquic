...
xqc_tp_test_make_conn(xqc_conn_type_t role, xqc_cid_t *out_peer_scid)
{
    ...
    if (role == XQC_CONN_TYPE_SERVER) {
        /* on a real server, conn->original_dcid stores the SCID of the
         * client's first Initial — populate it here so that an ISCID echoed
         * back by the (simulated) client can be compared against it. */
        xqc_cid_copy(&conn->original_dcid, &peer_scid);
    } else {
